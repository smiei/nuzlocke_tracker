...
export type EvolutionMethod =
  ...
  | { kind: "levelHeld"; item: string; time?: string | null }
  ...
  // Level-up variants PokeAPI spells out and this app used to discard as
  // "other" (which rendered as no condition at all): knowing a move, holding
  // an item, having a species in the party, or standing at a location.
  | { kind: "levelMove"; move: string; time?: string | null }
  | { kind: "levelParty"; species: string }
  | { kind: "levelLocation"; location: string }
  // Shedinja - appears in a free party slot rather than evolving.
  | { kind: "shed" }
...
// A handful of species had their catch rate CHANGED in Omega Ruby/Alpha
// Sapphire, and PokeAPI (where catchrates.json comes from) serves those
// post-ORAS values - which are wrong for every game this app covers. The
// worst offender is Rayquaza: 3 in Ruby through Black/White, raised to 45 in
// ORAS because the story forces you to catch it. Hand-curated in
// data/catchrate-history.json using the same maxGeneration convention as
// data/move-type-history.json ("this value applied through that generation,
// inclusive").
type CatchRateHistoryEntry = { id: number; catchRate: number; maxGeneration: number };

export function getCatchRateHistory(): CatchRateHistoryEntry[] {
  try {
    return readJson<CatchRateHistoryEntry[]>("catchrate-history.json");
  } catch {
    return [];
  }
}

// `generation` selects the era: omitted = PokeAPI's current values.
export function getCatchRates(generation?: number): CatchRateEntry[] {
  const rates = readJson<CatchRateEntry[]>("catchrates.json");
  if (generation === undefined) return rates;
  const overrides = new Map(
    getCatchRateHistory()
      .filter((entry) => generation <= entry.maxGeneration)
      .map((entry) => [entry.id, entry.catchRate]),
  );
  if (overrides.size === 0) return rates;
  return rates.map((rate) =>
    overrides.has(rate.id) ? { ...rate, catch_rate: overrides.get(rate.id)! } : rate,
  );
}
...
// `generation` rolls the NAMES back to what that generation's games actually
// printed (see MoveNameHistoryEntry); omitted = PokeAPI's current names.
export function getMoves(lang?: Lang, generation?: number): MovesTable {
  try {
    const raw = readJson<Record<string, MoveInfoFile>>("moves.json");
    const nameHistory = generation === undefined ? [] : getMoveNameHistory();
    ...
    for (const [slug, info] of Object.entries(raw)) {
      ...
      const names =
        nameHistory.length > 0
          ? historicalMoveNames(nameHistory, slug, generation!, rest.names)
          : rest.names;
      const entry = names === rest.names ? rest : { ...rest, names };
      out[slug] = text ? { ...entry, flavor: text } : entry;
    }
    return out;
  } catch {
    return {};
...
// Hand-curated corrections for moves whose German name changed in a later
// generation than the ones this app covers - see MoveNameHistoryEntry.
export function getMoveNameHistory(): MoveNameHistoryEntry[] {
  try {
    return readJson<MoveNameHistoryEntry[]>("move-name-history.json");
  } catch {
    return [];
  }
}
