...
export type GameInfo = {
  ...
  nameLang?: Lang;
  // This pack's source game ships its OWN species table - types, base stats,
  // catch rates and weights that differ from this app's shared pokemon.json.
  // The value names data/shared/<value>-species.json (generated by
  // scripts/generate-infinite-fusion-species.mjs); entries there win over the
  // shared data for this pack only, species it doesn't list keep it. Missing
  // = every pack before Infinite Fusion, i.e. shared data as before.
  speciesData?: string;
...
// A pack-scoped species table (GameInfo.speciesData): the source game's own
// types/base stats/catch rates/weights, generated from that game's data.
// Applied on top of the shared pokemon.json in gameData.ts - never here, so
// every generation-keyed function above stays game-agnostic. A missing file
// yields [], i.e. the pack simply uses the shared data.
export type SpeciesDataEntry = {
  id: number;
  // The source game's own name for the species - not displayed, it makes the
  // generated file readable next to a diff.
  name: string;
  types: string[];
  stats: PokemonStats;
  catchRate: number;
...
export function getSpeciesData(name: string): SpeciesDataEntry[] {
  try {
    return readJson<{ species: SpeciesDataEntry[] }>(path.join("shared", `${name}-species.json`))
      .species;
  } catch {
    return [];
  }
