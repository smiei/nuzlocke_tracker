export const LANGS = ["de", "en", "fr", "es", "it"] as const;
export type Lang = (typeof LANGS)[number];

export function isLang(value: unknown): value is Lang {
  return typeof value === "string" && (LANGS as readonly string[]).includes(value);
}

export const LANG_COOKIE = "lang";

const de = {
  header: {
    titleShort: "SoulLink Tracker",
    titleFull: "SoulLink Nuzlocke Tracker",
  },
  nav: {
    tracker: "Encounter",
    links: "Team",
    pokedex: "Pokédex",
    typen: "Kampf & Fang",
    weaknesses: "Team-Schwächen",
    overview: "Übersicht",
    tms: "TMs",
    catchrate: "Catchrate",
    levelcaps: "Reise",
    rules: "Regeln",
  },
  theme: {
    toggleNeutral: "Design wechseln",
    toDark: "Zu dunklem Design wechseln",
    toLight: "Zu hellem Design wechseln",
  },
  language: {
    switchLabel: "Sprache wechseln",
  },
  dialog: {
    ok: "OK",
    cancel: "Abbrechen",
    confirm: "Bestätigen",
    saving: "Speichern…",
    saved: "Gespeichert",
  },
  menu: {
    copyRunLink: "Link zum Run kopieren",
    runLinkCopied: "Link kopiert. Wer ihn hat, kann den Run ansehen und bearbeiten.",
    runLinkManual: (url: string) => `Link zum Run - wer ihn hat, kann den Run ansehen und bearbeiten:\n\n${url}`,
    label: "Menü",
    language: "Sprache",
    version: "Version",
    credits: "Credits",
    creditsSprites: "Fusions-Sprites",
    creditsSpritesSource: "Sprite-Community von Infinite Fusion (per Hotlink)",
    creditsData: "Routen & Fundorte",
    creditsMechanics: "Ball-Mechanik",
  },
  install: {
    menuLabel: "App installieren",
    iosTitle: "App installieren",
    iosHint:
      "Tippe in Safari unten auf „Teilen“ und dann auf „Zum Home-Bildschirm“. " +
      "Hinweis: Die installierte App hat unter iOS einen eigenen Speicher – Design, " +
      "Tab-Reihenfolge und Sprache starten dort einmalig auf den Standardwerten.",
    manualHint:
      "Dein Browser bietet keine direkte Installation an. Öffne das Browsermenü und " +
      "wähle „Zum Startbildschirm hinzufügen“ bzw. „Installieren“.",
  },
  blindflug: {
    name: "Blindflug",
    hidden: "Im Blindflug ausgeblendet",
    enable: "Blindflug einschalten",
    disable: "Blindflug ausschalten",
  },
  appearance: {
    label: "Darstellung",
    title: "Darstellung",
    sizeLabel: "Größe der Oberfläche",
    smaller: "Klein",
    normal: "Normal",
    larger: "Groß",
    preview: "So sieht Fließtext in dieser Größe aus.",
    previewSmall: "Und so die kleinen Hinweiszeilen darunter.",
    hint: "Gilt nur auf diesem Gerät.",
    reset: "Zurücksetzen",
    done: "Fertig",
  },
  tabOrder: {
    label: "Tabs anordnen",
    title: "Tabs anordnen",
    moveUp: "Nach oben",
    moveDown: "Nach unten",
    reset: "Zurücksetzen",
    done: "Fertig",
    hide: "Tab ausblenden",
    show: "Tab einblenden",
    visibilityHint: "Häkchen entfernen, um einen Tab auszublenden.",
  },
  backup: {
    menuLabel: "Backup & Import",
    backupRun: "Aktuellen Run sichern",
    backupAll: "Alle Runs sichern",
    import: "Backup importieren …",
    importSuccess: (n: number) => `${n} Run(s) importiert.`,
    importNewerVersion:
      "Dieses Backup wurde mit einer neueren App-Version erstellt. Manche Daten (z. B. Fusionen) werden beim Import möglicherweise nicht übernommen.",
    noRun: "Kein Run ausgewählt.",
    importTitle: "Backup importieren",
    importIntro:
      "Vergib für jeden enthaltenen Run einen Namen, bevor er als neuer Run angelegt wird.",
    importNameLabel: (i: number, total: number) =>
      total > 1 ? `Name für Run ${i + 1} von ${total}` : "Name für den neuen Run",
    importOriginalName: (name: string) => `Im Backup: „${name}"`,
    importUseBackupName: "Namen aus Backup übernehmen",
    importNameCollision: "Ein Run mit diesem Namen existiert bereits.",
    importConfirm: "Importieren",
    cancel: "Abbrechen",
  },
  landing: {
    title: "Nuzlocke Tracker",
    empty: "Auf diesem Gerät ist noch kein Run geöffnet.",
    hint: "Lege einen neuen Run an. Für einen SoulLink öffnest du einfach den Link, den dir deine Mitspieler geschickt haben.",
    create: "Neuen Run anlegen",
    linkNote: "Jeder Run hat einen eigenen, geheimen Link. Wer ihn kennt, kann den Run ansehen und bearbeiten - teile ihn nur mit deinen Mitspielern.",
  },
  runSwitcher: {
    newRun: "+ Neuer Run",
    rename: "Umbenennen",
    renameTitle: "Run umbenennen",
    renameLabel: (name: string) => `Run "${name}" umbenennen`,
    save: "Speichern",
    deleteButton: "Run löschen",
    deleteLabel: (name: string) => `Run "${name}" löschen`,
    newRunTitle: "Neuer Run",
    nameLabel: "Name",
    namePlaceholder: "z.B. Playthrough 2",
    gameLabel: "Spiel",
    modeLabel: "Modus",
    modeSoullink: "SoulLink",
    modeClassic: "Solo",
    cancel: "Abbrechen",
    create: "Erstellen",
    confirmDelete1: (name: string) =>
      `Run "${name}" wirklich löschen? Alle Encounters, Links und der Level-Cap-Fortschritt dieses Runs gehen unwiderruflich verloren.`,
    soloSuffix: " (Solo)",
    playersLabel: "Spieler",
    playersSuffix: (n: number) => ` (${n} Spieler)`,
  },
  player: {
    PLAYER1: "Spieler 1",
    PLAYER2: "Spieler 2",
    PLAYER3: "Spieler 3",
    PLAYER4: "Spieler 4",
  },
  status: {
    CAUGHT: "Gefangen",
    KILLED: "Getötet",
    FLED: "Geflohen",
  },
  tracker: {
    custom: {
      manage: "Eigene Routen",
      title: "Eigene Routen",
      nameLabel: "Name",
      namePlaceholder: "z. B. Safari-Zone (Extra)",
      typeLabel: "Art",
      typeRoute: "Route",
      typeStatic: "Static",
      positionLabel: "Einfügen nach",
      positionTop: "(ganz oben)",
      add: "Hinzufügen",
      existing: "Eigene Routen in diesem Run",
      none: "Noch keine eigenen Routen.",
      hint: "Zählt im Fortschrittsbalken mit und gilt nur für diesen Run.",
      delete: "Löschen",
      deleteConfirm: (n: number) =>
        n === 0 ? "Wirklich löschen?" : `Wirklich löschen? ${n} Encounter gehen mit.`,
      deleteYes: "Ja, löschen",
      added: (name: string) => `„${name}“ hinzugefügt.`,
      deleted: (name: string) => `„${name}“ gelöscht.`,
    },
    debug: {
      export: "Reihenfolge exportieren",
      title: "Encounter-Reihenfolge",
      hint: "Zum Korrigieren der routes.json des Spielpakets – Text kopieren und an Claude Code schicken.",
      copy: "Kopieren",
      copied: "Kopiert.",
      copyFailed: "Kopieren nicht möglich – Text bitte von Hand markieren.",
      download: "Als Datei speichern",
    },
    heading: "Encounter",
    searchPlaceholder: "Pokémon suchen...",
    noResults: "Keine Treffer",
    locked: "gesperrt",
    clearSelection: "Auswahl leeren",
    shinyToggle: "Als Shiny markieren (Shiny Clause)",
    bodyOfFusion: (head: string, route: string) => `Körper von ${head} (${route})`,
    fusedWith: (body: string, route: string) => `Fusioniert mit ${body} (${route})`,
    caughtAsFusion: "Als Fusion gefangen",
    typeStatic: "Static",
    openOnly: "Nur offene",
    openOnlyTitle: "Nur Routen ohne vollständigen Eintrag anzeigen",
    missingPlayer: "Encounter fehlt noch für mindestens einen Spieler",
    clear: "Leeren",
    clearConfirm: "Diesen Encounter wirklich löschen?",
    nicknamePlaceholder: "Spitzname",
    nicknameLabel: "Spitzname",
    postgameShow: (n: number) => `Post-Game Content anzeigen (${n} Gebiete)`,
    postgameHide: "Post-Game Content ausblenden",
    allDone: "Alle Routen erledigt",
    showAll: "Alle anzeigen",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} von ${total} Routen abgeschlossen (${percent}%)`,
  },
  links: {    free: {
      add: "Pokémon hinzufügen",
      title: "Pokémon ins Team",
      pokemonLabel: "Pokémon",
      playerLabel: "Für",
      nicknameLabel: "Spitzname",
      submit: "Hinzufügen",
      added: (name: string) => `${name} hinzugefügt.`,
      removed: (name: string) => `${name} entfernt.`,
      remove: "Entfernen",
      removeConfirm: "Wirklich entfernen?",
      removeYes: "Ja, entfernen",
      hint: "Legt im Hintergrund einen eigenen Eintrag an. Zählt nicht im Encounter-Fortschritt.",
    },

    emptySoullink: "Noch keine SoulLinks – fange im Encounter-Tab dein erstes Pokémon.",
    emptyClassic: "Noch keine Pokémon – fange im Encounter-Tab dein erstes Pokémon.",
    markDead: "Als tot markieren",
    clearTeam: "Team leeren",
    clearTeamConfirm: "Alle sechs Team-Plätze leeren? Die Pokémon selbst bleiben erhalten.",
    dead: "Tot",
    deadBy: (name: string) => `verloren durch ${name}`,
    whoLost: "Wer hat verloren?",
    noAttribution: "Ohne Zuordnung",
    deathCausePlaceholder: "Todesursache (optional)",
    revive: "Wiederbeleben",
    evolve: "Entwickeln",
    revert: "Zurückentwickeln",
    staticTag: "static",
    rankSummary: (rang: number, summe: number) => `Rang #${rang} · BST ${summe}`,
    fusionSummary: (summe: number) => `Fusion · BST ${summe}`,
    fuse: "Fusionieren",
    fuseDialogTitle: "Fusionieren",
    fuseDialogHint:
      "Verschmilzt diesen Fang (Kopf) mit einem anderen eigenen Fang (Körper). Beide bleiben eigene Fänge, nur die Karte zeigt sie fortan zusammen.",
    fuseDonorLabel: "Körper",
    fuseNoCandidates: "Keine anderen gefangenen Pokémon zum Fusionieren.",
    fuseNoCustomSprite: "Für diese Kombination gibt es kein handgezeichnetes Sprite, nur ein automatisch generiertes.",
    fuseCdnUnreachable: "Sprite-Server nicht erreichbar.",
    fuseSubmit: "Fusionieren",
    fuseSuccess: "Fusioniert.",
    swap: "Tauschen",
    swapTitle: "Kopf und Körper tauschen",
    unfuse: "Lösen",
    unfuseConfirm: "Fusion wirklich lösen?",
    fusionTag: "Fusion",
    bodyLabel: "Körper",
    slotTakenBy: (routes: string) => `Belegt von ${routes}`,
    slotTakenHint:
      "Diese Gruppe braucht einen weiteren Platz, solange ein Spieler mehr einzelne Pokémon darin hat als der andere.",
    sortLabel: "Sortierung",
    sortDefault: "Encounter-Reihenfolge",
    sortSumme: "BST",
    sortSummeMax: "Max BST",
    filterEvolvable: "Entwickelbar",
    filterEvolvableTitle: "Nur Pokémon mit jetzt möglicher Level-Entwicklung anzeigen",
    hideTeam: "Team ausblenden",
    hideTeamTitle: "Pokémon im Team ausblenden",
    filterTypes: "Typen",
    filterTypesTitle: "Nach Typ filtern",
    filterTypesClear: "Zurücksetzen",
    noMatches: "Keine Pokémon passen zu den Filtern",
    teamHeading: "Aktuelles Team",
    teamSlotLabel: (n: number) => `Team-Platz ${n}`,
    teamEmpty: "Leer",
    teamSelectPlaceholder: "— Leer —",
    addToTeam: "Ins Team",
    replaceHint: "Wen ersetzen?",
  },
  pokedex: {
    heading: "Pokédex",
    searchPlaceholder: "Pokémon suchen...",
    noResults: "Keine Treffer",
    filterAll: "Alle",
    filterAvailable: "Nur verfügbare",
    filterLocked: "Nur gesperrte",
    filterLegendary: "Nur Legendäre",
      resetFilters: "Filter zurücksetzen",
    columns: {
      id: "ID",
      name: "Name",
      types: "Typen",
      kp: "KP",
      ang: "Ang.",
      vert: "Vert.",
      spA: "Sp.-A.",
      spV: "Sp.-V.",
      spez: "Spezial",
      init: "Init.",
      rang: "Rang",
      summe: "BST",
    },
    detail: {
      rank: "Rang",
      weaknesses: "Typ-Schwächen",
      stats: "Statuswerte",
      moves: "Attacken (nach Level)",
      evolution: "Entwicklung",
      noEvolution: "Keine Entwicklung",
      fusionHead: "Kopf",
      fusionBody: "Körper",
      noMoveData: "Für dieses Spiel sind keine Attackendaten hinterlegt.",
      evolvesFrom: "Entwickelt sich aus",
      level: (n: number) => `Lv. ${n}`,
      movePower: "Stärke",
      moveAccuracy: "Genauigkeit",
      movePp: "AP",
      moveCategory: "Kategorie",
      moveEffect: "Effekt",
      moveChance: (n: number) => `${n} % Chance`,
      // Quiet hint when this generation's value differs from the modern one.
      moveToday: (value: string) => `heute ${value}`,
      moveClass: { physical: "Physisch", special: "Speziell", status: "Status" },
      moveDetails: "Details ein-/ausblenden",
      weight: "Gewicht",
      dexNo: "Nr.",
      forms: "Formen",
      formBase: "Normalform",
      openInAnalyze: "Im Kampf & Fang öffnen",
    },
  },
  errorPage: {
    title: "Da ist etwas schiefgelaufen",
    hint: "Die Seite konnte nicht geladen werden. Ein neuer Versuch hilft meistens.",
    retry: "Erneut versuchen",
  },
  levelcaps: {
    heading: "Reise",
    maxLevel: "Max. Level",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} von ${total} Kämpfen abgeschlossen (${percent}%)`,
    eliteFourMarker: "Ab hier: Top 4",
    empty: "Keine Kämpfe in diesem Spielpaket hinterlegt.",
  },
  weaknesses: {
    heading: "Team-Schwächen",
    empty: "Kein Team aufgestellt – wähle im Pokémon-Tab bis zu 6 Pokémon für dein Team.",
    attackType: "Angriffstyp",
    weakHeader: "Schwach",
    resistHeader: "Resistent",
    criticalHint:
      "⚠ = kritische Schwäche: Der Angriffstyp trifft mindestens die Hälfte des Teams effektiv, ohne dass jemand resistiert.",
  },
  rules: {
    debugHeading: "Debug",
    heading: "Regeln",
    edit: "Bearbeiten",
    save: "Speichern",
    cancel: "Abbrechen",
    loadDefault: "Standard laden",
    loadDefaultConfirm: "Das aktuelle Regelwerk durch das Standard-Regelwerk ersetzen? (Wird erst mit „Speichern“ übernommen.)",
    settingsHeading: "Regel-Einstellungen",
    playerNamesHeading: "Spielernamen",
    playerNamesHint: "Ersetzt „Spieler 1“, „Spieler 2“ … in diesem Run.",
    presets: {
      label: "Regelwerk",
      placeholder: "Regelwerk laden…",
      manage: "Regelwerke",
      dialogTitle: "Regelwerke",
      nameLabel: "Name des Regelwerks",
      namePlaceholder: "z. B. SoulLink Standard",
      saveCurrent: "Speichern",
      overwrite: "Überschreiben",
      overwriteHint: (name: string) =>
        `„${name}“ existiert bereits und wird ersetzt.`,
      existing: "Gespeicherte Regelwerke",
      none: "Noch keine Regelwerke gespeichert.",
      scopeHint: "Regelwerke gelten app-weit und stehen in jedem Run zur Auswahl.",
      editingHint: "Notizen erst speichern.",
      delete: "Löschen",
      deleteConfirm: "Wirklich löschen?",
      deleteYes: "Ja, löschen",
      applyConfirm: (name: string) =>
        `„${name}“ laden? Regel-Einstellungen und Hausregeln dieses Runs werden überschrieben.`,
      applied: (name: string) => `„${name}“ geladen.`,
      saved: (name: string) => `„${name}“ gespeichert.`,
      deleted: (name: string) => `„${name}“ gelöscht.`,
    },
    notesHeading: "Hausregeln & Notizen",
    showNotes: "Einblenden",
    hideNotes: "Ausblenden",
    toggles: {
      freeTeam: {
        label: "Freies Team (kein Nuzlocke)",
        description:
          "Für ganz normale Durchgänge: der Team-Tab bekommt einen „Pokémon hinzufügen“-Knopf, statt dass jedes Teammitglied über eine Route eingetragen werden muss. Der Encounter-Tab bleibt nutzbar.",
      },
      customSpritesOnly: {
        label: "Nur handgezeichnete Sprites",
        description:
          "Beim Fusionieren erscheint eine Warnung, wenn die gewählte Kombination kein handgezeichnetes Sprite hat, nur ein automatisch generiertes. Blockiert das Fusionieren nicht.",
      },
      randomizerFuseEverything: {
        label: "Randomizer: Alles fusioniert",
        description:
          "Für Runs mit der Randomizer-Option „Fuse everything“: Jedes wilde Pokémon ist schon eine Fusion, deshalb steht im Encounter-Tab das Körperfeld sofort offen, ohne Häkchen. Bleibt es leer, ist der Fang keine Fusion.",
      },
      speciesClausePerPlayer: {
        label: "Species Clause pro Spieler",
        description:
          "Jeder Spieler hat seine eigene Species Clause: Ein Encounter sperrt die Familie nur für den Spieler, der ihn hatte – egal ob gefangen, entkommen oder besiegt.",
      },
      speciesClauseLinkedLocksAll: {
        label: "Gefangene Links sperren für alle",
        description:
          "Nur mit „pro Spieler“: Kommt ein Link zustande, weil alle Spieler auf der Route fangen, ist die Familie für alle Spieler verbraucht. Entkommene oder besiegte Pokémon sperren weiter nur den eigenen Spieler.",
      },
      wildFusionSplit: {
        label: "Wilde Fusionen dürfen getrennt werden",
        description:
          "Ein Fang, der schon als Fusion gefangen wurde, lässt sich wieder in zwei Pokémon trennen – wie im Spiel. Aus = so ein Fang bleibt dauerhaft eine Einheit. Selbst fusionierte Paare sind davon nicht betroffen.",
      },
      fusionLocksBothFamilies: {
        label: "Wilde Fusionen sperren beide Familien",
        description:
          "Eine wild gefangene Fusion sperrt die Evolutionsfamilien beider Arten, so wie zwei einzelne Fänge. Aus = nur der Kopf sperrt.",
      },
      blindflug: {
        label: "Blindflug",
        description:
          "Blendet alles aus, was die App für dich ausrechnet: Typeneffektivität an jeder Stelle sowie Attackenlisten und -beschreibungen. Die Typentabelle selbst bleibt. Gilt für den ganzen Run.",
      },
      debugMode: {
        label: "Debug: Encounter-Reihenfolge aufzeichnen",
        description:
          "Blendet im Encounter-Tab einen Export der Reihenfolge ein, in der Routen eingetragen wurden – zum Korrigieren der routes.json eines Spielpakets. Aufgezeichnet wird immer; der Schalter blendet nur den Export ein und gilt nur auf diesem Gerät.",
      },
      speciesClause: {
        label: "Species Clause",
        description:
          "Warnt, wenn eine Evolutionsfamilie im Run bereits verwendet wurde. Blockiert nie das Speichern.",
      },
      shinyClause: {
        label: "Shiny Clause",
        description:
          "Ein als Shiny markierter Fang ist von der Species Clause ausgenommen (Shinys dürfen immer gefangen werden).",
      },
      staticsExemptFromClause: {
        label: "Statics von der Species Clause ausgenommen",
        description:
          "Auf Static-Routen (Geschenke, Fossilien, Legendäre) wird keine Clause-Warnung angezeigt.",
      },
      nicknames: {
        label: "Spitznamen",
        description:
          "Eingabefeld im Encounter-Tab; Karten im Pokémon-Tab zeigen den Spitznamen an.",
      },
      evolutionOverridesImpossible: {
        label: "Randomizer: Unmögliche Entwicklungen geändert",
        description:
          "Ersetzte Tausch- & Co.-Entwicklungen aus evolution-overrides.json (z. B. Kadabra → ab Level 37). Aus = Original-Methoden.",
      },
      evolutionOverridesEasier: {
        label: "Randomizer: Entwicklungen erleichtert",
        description:
          "Gesenkte Entwicklungslevel aus evolution-overrides.json (z. B. Dragonir → ab Level 40). Aus = Original-Level.",
      },
      evolutionOverridesTimeBased: {
        label: "Randomizer: Zeitbasierte Entwicklungen entfernt",
        description:
          "Tageszeit-Bedingungen entfernt (Evoli → Psiana/Nachtara per Sonnen-/Mondstein, Glücklichkeits- und Item-Entwicklungen ohne Tag/Nacht). Aus = Original-Bedingungen.",
      },
      statics: {
        label: "Static-Encounter",
        description:
          "Zeigt Static-Standorte im Encounter-Tab. Aus = nur reguläre Routen werden getrackt.",
      },
    },
  },
  catchrate: {
    heading: "Catchrate",
    hint: "Wähle ein Pokémon, um die Fangchance zu berechnen.",
    lockWarning: "Dieses Pokémon ist in diesem Run durch die Species Clause gesperrt.",
    lockWarningPlayer: "Für diesen Spieler durch die Species Clause gesperrt.",
    caughtHeading: "Pokémon gefangen?",
    caughtNeedSelection: "Erst oben ein Pokémon auswählen.",
    caughtNoRoutes: "Keine offenen Routen mehr.",
    selectRoute: "Route wählen …",
    caughtDone: (name: string, route: string) => `${name} auf ${route} als gefangen eingetragen.`,
    confirmCatch: "Fangen",
    conditionMet: "Bedingung erfüllt",
    addCard: "Weitere Kachel",
    ballLabel: "Pokéball",
    hpLabel: "KP (%)",
    levelLabel: "Level des wilden Pokémon",
    ownLevelLabel: "Level deines Pokémon",
    statusLabel: "Statusveränderung",
    turnLabel: "Kampfrunde",
    resultLabel: "Fangchance pro Wurf",
    guaranteed: "Garantierter Fang",
    avgThrows: (n: string) => `Ø ${n} Würfe bis zum Fang`,
    details: (rate: number, ball: string, status: string) =>
      `Basis-Fangrate ${rate} · Ball ${ball} · Status ${status}`,
    balls: {
      poke: "Pokéball",
      great: "Superball",
      ultra: "Hyperball",
      master: "Meisterball",
      safari: "Safariball",
      net: "Netzball",
      nest: "Nestball",
      dive: "Tauchball",
      repeat: "Wiederball",
      timer: "Timerball",
      luxury: "Luxusball",
      premier: "Premierball",
      level: "Levelball",
      lure: "Köderball",
      moon: "Mondball",
      friend: "Freundesball",
      love: "Sympaball",
      fast: "Turboball",
      park: "Parkball",
      quick: "Flottball",
      dusk: "Finsterball",
      heal: "Heilball",
      dream: "Traumball",
      heavy: "Schwerball",
      sport: "Turnierball",
      // Infinite Fusion only. The game's own balls exist in English alone,
      // so they keep that name in every language.
      cherish: "Jubelball",
      gender: "Gender Ball",
      boost: "Boost Ball",
      ability: "Ability Ball",
      virus: "Virus Ball",
      glitter: "Glitter Ball",
      perfect: "Perfect Ball",
      toxic: "Toxic Ball",
      spark: "Spark Ball",
      scorch: "Scorch Ball",
      frost: "Frost Ball",
      pure: "Pure Ball",
      status: "Status Ball",
      candy: "Candy Ball",
      rocket: "Rocket Ball",
      fusion: "Fusion Ball",
    },
    statusOptions: {
      none: "Keine",
      sleep: "Schlaf",
      freeze: "Gefroren",
      paralysis: "Paralyse",
      poison: "Gift",
      burn: "Verbrennung",
    },
    ballNotes: {
      net: "Netzball: 3× gilt nur gegen Wasser- und Käfer-Pokémon (wird automatisch anhand des Typs berücksichtigt).",
      nest: "Nestball: Bonus hängt vom Level des wilden Pokémon ab (bis ×3,9 bei Level 1, ab Level 30 kein Bonus).",
      dive: "Tauchball: 3,5× gilt nur beim Surfen/Tauchen/Angeln – hier angenommen.",
      repeat: "Wiederball: 3× gilt nur, wenn die Spezies bereits im Pokédex registriert ist – hier angenommen.",
      timer: "Timerball: Bonus wächst mit der Kampfrunde (max. ×4 ab Runde 30).",
      level: "Levelball: ×2, wenn dein Pokémon ein höheres Level hat als das wilde, ×4 ab doppeltem und ×8 ab vierfachem Level.",
      lure: "Köderball: ×3 gilt nur beim Angeln – hier angenommen.",
      moon: "Mondball: ×4 gilt nur bei Mondstein-Familien (z. B. Nidoran, Piepi) – hier angenommen.",
      friend: "Freundesball: kein Fangbonus, erhöht nach dem Fang die Freundschaft.",
      love: "Sympaball: ×8 gilt nur bei gleicher Art mit entgegengesetztem Geschlecht – hier angenommen.",
      fast: "Turboball: ×4 gilt nur bei fluchtfreudigen Arten – hier angenommen.",
      park: "Parkball: ×1,5 beim Käferturnier im Nationalpark.",
      quick: "Flottball: ×4 nur in Runde 1, danach ×1.",
      dusk: "Finsterball: ×3,5 gilt nur nachts oder in Höhlen – hier angenommen.",
      heal: "Heilball: kein Fangbonus, heilt das gefangene Pokémon vollständig.",
      dream: "Traumball: fängt in Gen 5 garantiert (nur in der Traumwelt erhältlich).",
      heavy: "Schwerball: verschiebt die Fangrate je nach Gewicht (−20 unter 102 kg, bis +40 ab 410 kg) statt sie zu vervielfachen.",
      sport: "Turnierball: kein Fangbonus, nur beim Käferturnier erhältlich.",
    },
    // Infinite Fusion only (see catchrate.ts): win over ballNotes where the
    // game's own code differs from the real games, plus its own balls.
    ballNotesInfiniteFusion: {
      nest: "Nestball: Bonus nur bis Level 30, (41 − Level) ÷ 10 – bei Level 1 also ×4.",
      timer: "Timerball: ×1 in Runde 1, danach +0,3 je Runde, max. ×4.",
      quick: "Flottball: ×5 nur in Runde 1, danach ×1.",
      fast: "Turboball: ×4 gegen Pokémon mit Basis-Initiative ab 100 (wird automatisch berücksichtigt).",
      heavy: "Schwerball: −20 unter 204,8 kg, +20 ab 204,8 kg, +30 ab 307,2 kg, +40 ab 409,6 kg. Eine Fusion wiegt den Durchschnitt beider Pokémon.",
      sport: "Turnierball: ×1,5.",
      dream: "Traumball: schläfert das Pokémon beim Wurf ein, es gilt also der Schlaf-Bonus ×2,5. Kein garantierter Fang.",
      cherish: "Jubelball: kein Fangbonus.",
      gender: "Gender Ball: kein Fangbonus, tauscht nach dem Fang das Geschlecht.",
      boost: "Boost Ball: ×0,8, das Pokémon zählt nach dem Fang als getauscht.",
      ability: "Ability Ball: ×0,6, das Pokémon erhält seine versteckte Fähigkeit.",
      virus: "Virus Ball: ×0,4, das Pokémon bekommt Pokérus.",
      glitter: "Glitter Ball: ×0,2, das Pokémon wird schillernd (Shiny).",
      perfect: "Perfect Ball: ×0,1, ein oder zwei zufällige IVs werden auf 31 gesetzt.",
      toxic: "Toxic Ball: vergiftet das Pokémon beim Wurf, es gilt der Status-Bonus ×1,5.",
      spark: "Spark Ball: paralysiert das Pokémon beim Wurf, es gilt der Status-Bonus ×1,5.",
      scorch: "Scorch Ball: verbrennt das Pokémon beim Wurf, es gilt der Status-Bonus ×1,5.",
      frost: "Frost Ball: friert das Pokémon beim Wurf ein, es gilt der Status-Bonus ×2,5.",
      pure: "Pure Ball: soll ohne Statusproblem ×3,5 geben. Laut Spielcode greift der Bonus aber nie, daher hier ×1.",
      status: "Status Ball: soll mit Statusproblem ×2,5 geben. Laut Spielcode greift der Bonus aber immer, daher hier stets ×2,5.",
      candy: "Candy Ball: ×0,8, das Pokémon ist nach dem Fang 5 Level höher.",
      rocket: "Rocket Ball: kein Fangbonus.",
      fusion: "Fusion Ball: ×3 gegen Fusionen (wird automatisch berücksichtigt, sobald ein Körper gewählt ist).",
    },
    lastBallLabel: "Letzter Ball dieser Sorte im Beutel",
    lastBallHint: "Dann ist ein kritischer Fang möglich: ein einziger Schüttel-Check statt vier.",
    dexOwnedLabel: "Gefangene Arten im Pokédex",
    dexOwnedOptions: ["bis 300", "301–450", "451–600", "über 600"],
    caughtNoFusion:
      "Fusionen lassen sich hier nicht eintragen – den Kopf im Encounter-Tab eintragen und im Team-Tab fusionieren.",
  },
  typen: {
    heading: "Typeneffektivität",
    battleHeading: "Kampf",
    analyzeHeading: "Kampf & Fang",
    viewWild: "Wild",
    viewTrainer: "Trainer",
    battleHint: "Wähle einen Gegner, um seine Typen, Attacken und den Team-Vergleich zu sehen.",
    levelLabel: "Level",
    moveTypes: "Attacken-Typen",
    damagingOnly: "nur Schadensattacken",
    bodyPickerLabel: "Körper (optional, für Fusionen)",
    noMoveData: "Für dieses Spiel sind keine Attackendaten hinterlegt.",
    teamMatchup: "Wie gut hält dein Team stand?",
    explosionWarn: (name: string, lvl: number) => `⚠ Kann ab Lv ${lvl} ${name} einsetzen!`,
    hint: "Wähle ein Pokémon, um seine Schwächen und Resistenzen zu sehen.",
    weak4: "4× Schwäche gegen",
    weak2: "2× Schwäche gegen",
    resist2: "0.5× Resistenz gegen",
    resist4: "0.25× Starke Resistenz gegen",
    immune: "0× Immun gegen",
    attack: "Angriff",
    defense: "Verteidigung",
    statsHeading: "Stats",
    defenderWeakHeading: "Schwächen als Verteidiger",
    teamStrengthHeading: "Stärken gegen mein Team",
    matrixHeading: "Typen-Matrix",
    legendOffense: "Grün = du triffst stark · Rot = du triffst schwach · Schwarz = wirkungslos",
    legendDefense: "Grün = dein Team nimmt wenig · Rot = dein Team nimmt viel · Schwarz = immun",
    addCard: "Weiterer Gegner",
  },
  tms: {
    heading: "TM-Kompatibilität",
    searchPlaceholder: "Attacke suchen …",
    noResults: "Keine Attacke gefunden",
    hint: "Wähle eine Attacke, um zu sehen, welche deiner Pokémon sie offiziell lernen können.",
    caveat: "Quelle: offizielle Spieldaten (Original-Kompatibilität) – nicht die randomisierte ROM-Zuordnung.",
    notLearnable: "Diese Attacke ist im Original keine TM/VM- oder Tutor-Attacke.",
    empty: "Noch keine lebenden Pokémon gefangen.",
    teamHeading: "Team",
    bankHeading: "Bank",
    method: { tm: "TM", hm: "VM", tutor: "Tutor", level: "Lv." },
    noMoveData: "Für dieses Spiel sind keine TM- und Attackendaten hinterlegt.",
  },
  overview: {
    heading: "Übersicht",
    progress: "Fortschritt",
    encounterProgress: "Encounter",
    battleProgress: "Reise",
    badgesHeading: "Orden",
    badgesEmpty: "Für dieses Spiel sind keine Orden hinterlegt.",
    dashboard: "Dashboard",
    coverage: "Team-Abdeckung",
    offensiveGaps: "Offensive Lücken",
    offensiveGapsInfo: (level: number) =>
      `Berücksichtigt werden nur Angriffstypen, die das Team bis zum zuletzt erreichten Level-Cap (Level ${level}) tatsächlich erlernt hat.`,
    noGaps: "Keine Lücken – das Team trifft jeden Typ sehr effektiv.",
    offensiveHint: "Typen, die kein Team-Mitglied sehr effektiv trifft.",
    memorial: "Friedhof",
    memorialEmpty: "Noch keine Verluste. Bleib vorsichtig.",
    deathPoint: "Todeszeitpunkt",
    deathPointNone: "Kein Zeitpunkt",
    deathPointUnset: "Zeitpunkt eintragen …",
    totalDeaths: "Tode gesamt",
    teamBst: "Team-BST (jetzt → max)",
    levelCap: "Level-Cap (jetzt → nächstes)",
    deathTallyHeading: "Verantwortlich für Tode",
    deathTallyUnattributed: (n: number) => `+${n} ohne Zuordnung`,
    player: "Spieler",
    team: "Team",
    caught: "Gefangen",
    bankBstCol: "Bank-BST",
    bankBstColHint: "BST der gefangenen, lebenden Pokémon außerhalb des Teams (jetzt → voll entwickelt)",
    teamBstCol: "Team-BST",
    teamBstColHint: "BST der Team-Pokémon (jetzt → voll entwickelt)",
    emptyRun: "Noch nichts gefangen.",
    emptyRunHint: "Sobald du im Encounter-Tab etwas fängst, erscheinen hier BST, Abdeckung und Statistiken.",
    bankBst: "Bank-BST (jetzt → max)",
    caughtTotal: "Gefangen",
  },
  actions: {
    unknownPokemon: (id: number) => `Unbekannte Pokémon-ID: ${id}`,
    unknownRoute: (id: number) => `Unbekannte Routen-ID: ${id}`,
    speciesLocked: (pokemonName: string, playerLabel: string, routeName: string) =>
      `${pokemonName} ist durch die Species Clause gesperrt (bereits von ${playerLabel} auf ${routeName} verwendet).`,
    playerNotInRun: "Diesen Spieler gibt es in diesem Run nicht.",
    encounterNotFound: (id: number) => `Encounter #${id} nicht gefunden.`,
    soulLinkNotFound: (id: number) => `SoulLink #${id} nicht gefunden.`,
    runNotFound: "Run nicht gefunden - der Link ist ungültig oder der Run wurde gelöscht.",
    notOnPublicInstance: "Das ist auf der öffentlichen Seite nicht verfügbar.",
    deadCannotEvolve: "Ein totes Pokémon kann nicht entwickelt werden.",
    deadCannotRevert: "Ein totes Pokémon kann nicht zurückentwickelt werden.",
    invalidEvolutionTarget: "Ungültiges Entwicklungsziel.",
    invalidFormTarget: "Ungültige Form für dieses Pokémon.",
    evolutionFamilyMismatch: "Interner Fehler: Familien-Inkonsistenz in evolutions.json.",
    noPreEvolution: "Keine Vorentwicklung vorhanden.",
    unknownLevelCap: (id: number) => `Unbekannter Level Cap: ${id}`,
    presetNotFound: (id: number) => `Unbekanntes Regelwerk: ${id}`,
    nameRequired: "Name darf nicht leer sein.",
    backupInvalid: "Ungültige oder beschädigte Backup-Datei.",
    backupEmpty: "Die Backup-Datei enthält keine Runs.",
    invalidTeamSlot: "Ungültiger Team-Platz.",
    fusionSameEncounter: "Kopf und Körper müssen zwei unterschiedliche Fänge sein.",
    fusionDifferentPlayer: "Kopf und Körper müssen demselben Spieler gehören.",
    fusionNotCaught: "Beide Fänge müssen lebend und gefangen sein, um sie zu fusionieren.",
    fusionChain: "Einer der beiden ist bereits Teil einer anderen Fusion.",
    fusionNotFound: "Diese Karte ist keine Fusion.",
    fusionDead: "Eine tote Fusion kann nicht verändert werden.",
    fusionSplitDisabled:
      "Die Regel „Wilde Fusionen dürfen getrennt werden“ ist in diesem Run aus.",
    unexpected: "Unerwarteter Fehler - Aktion wurde nicht ausgeführt. Details stehen im Server-Log.",
  },
};

export type Dictionary = typeof de;

const en: Dictionary = {
  header: {
    titleShort: "SoulLink Tracker",
    titleFull: "SoulLink Nuzlocke Tracker",
  },
  nav: {
    tracker: "Encounter",
    links: "Team",
    pokedex: "Pokédex",
    typen: "Battle & Catch",
    weaknesses: "Team Weaknesses",
    overview: "Overview",
    tms: "TMs",
    catchrate: "Catchrate",
    levelcaps: "Journey",
    rules: "Rules",
  },
  theme: {
    toggleNeutral: "Switch theme",
    toDark: "Switch to dark theme",
    toLight: "Switch to light theme",
  },
  language: {
    switchLabel: "Switch language",
  },
  dialog: {
    ok: "OK",
    cancel: "Cancel",
    confirm: "Confirm",
    saving: "Saving…",
    saved: "Saved",
  },
  menu: {
    copyRunLink: "Copy link to this run",
    runLinkCopied: "Link copied. Anyone who has it can view and edit the run.",
    runLinkManual: (url: string) => `Link to this run - anyone who has it can view and edit the run:\n\n${url}`,
    label: "Menu",
    language: "Language",
    version: "Version",
    credits: "Credits",
    creditsSprites: "Fusion sprites",
    creditsSpritesSource: "Infinite Fusion's sprite community (hotlinked)",
    creditsData: "Routes & locations",
    creditsMechanics: "Ball mechanics",
  },
  install: {
    menuLabel: "Install app",
    iosTitle: "Install app",
    iosHint:
      "In Safari, tap “Share” at the bottom, then “Add to Home Screen”. " +
      "Note: on iOS the installed app gets its own storage – theme, tab order and " +
      "language start out at their defaults once.",
    manualHint:
      "Your browser has no direct install option. Open the browser menu and choose " +
      "“Add to Home screen” or “Install”.",
  },
  blindflug: {
    name: "Blind flight",
    hidden: "Hidden in blind flight",
    enable: "Turn on blind flight",
    disable: "Turn off blind flight",
  },
  appearance: {
    label: "Appearance",
    title: "Appearance",
    sizeLabel: "Interface size",
    smaller: "Small",
    normal: "Normal",
    larger: "Large",
    preview: "This is what body text looks like at this size.",
    previewSmall: "And this is the small print underneath it.",
    hint: "Applies to this device only.",
    reset: "Reset",
    done: "Done",
  },
  tabOrder: {
    label: "Arrange tabs",
    title: "Arrange tabs",
    moveUp: "Move up",
    moveDown: "Move down",
    reset: "Reset",
    done: "Done",
    hide: "Hide tab",
    show: "Show tab",
    visibilityHint: "Uncheck to hide a tab.",
  },
  backup: {
    menuLabel: "Backup & Import",
    backupRun: "Back up current run",
    backupAll: "Back up all runs",
    import: "Import backup …",
    importSuccess: (n: number) => `${n} run(s) imported.`,
    importNewerVersion:
      "This backup was made with a newer app version. Some data (e.g. fusions) may not carry over on import.",
    noRun: "No run selected.",
    importTitle: "Import backup",
    importIntro: "Give each run in the backup a name before it's added as a new run.",
    importNameLabel: (i: number, total: number) =>
      total > 1 ? `Name for run ${i + 1} of ${total}` : "Name for the new run",
    importOriginalName: (name: string) => `In the backup: "${name}"`,
    importUseBackupName: "Use name from backup",
    importNameCollision: "A run with this name already exists.",
    importConfirm: "Import",
    cancel: "Cancel",
  },
  landing: {
    title: "Nuzlocke Tracker",
    empty: "No run is open on this device yet.",
    hint: "Create a new run. To join a SoulLink, just open the link your fellow players sent you.",
    create: "Create a new run",
    linkNote: "Every run has its own secret link. Anyone who knows it can view and edit the run - only share it with your fellow players.",
  },
  runSwitcher: {
    newRun: "+ New Run",
    rename: "Rename",
    renameTitle: "Rename run",
    renameLabel: (name: string) => `Rename run "${name}"`,
    save: "Save",
    deleteButton: "Delete Run",
    deleteLabel: (name: string) => `Delete run "${name}"`,
    newRunTitle: "New Run",
    nameLabel: "Name",
    namePlaceholder: "e.g. Playthrough 2",
    gameLabel: "Game",
    modeLabel: "Mode",
    modeSoullink: "SoulLink",
    modeClassic: "Solo",
    cancel: "Cancel",
    create: "Create",
    confirmDelete1: (name: string) =>
      `Really delete run "${name}"? All encounters, links, and level cap progress for this run will be lost permanently.`,
    soloSuffix: " (Solo)",
    playersLabel: "Players",
    playersSuffix: (n: number) => ` (${n} players)`,
  },
  player: {
    PLAYER1: "Player 1",
    PLAYER2: "Player 2",
    PLAYER3: "Player 3",
    PLAYER4: "Player 4",
  },
  status: {
    CAUGHT: "Caught",
    KILLED: "Killed",
    FLED: "Fled",
  },
  tracker: {
    custom: {
      manage: "Custom routes",
      title: "Custom routes",
      nameLabel: "Name",
      namePlaceholder: "e.g. Safari Zone (extra)",
      typeLabel: "Kind",
      typeRoute: "Route",
      typeStatic: "Static",
      positionLabel: "Insert after",
      positionTop: "(at the very top)",
      add: "Add",
      existing: "Custom routes in this run",
      none: "No custom routes yet.",
      hint: "Counts towards the progress bar and applies to this run only.",
      delete: "Delete",
      deleteConfirm: (n: number) =>
        n === 0 ? "Really delete?" : `Really delete? ${n} encounters go with it.`,
      deleteYes: "Yes, delete",
      added: (name: string) => `“${name}” added.`,
      deleted: (name: string) => `“${name}” deleted.`,
    },
    debug: {
      export: "Export order",
      title: "Encounter order",
      hint: "For correcting the game pack’s routes.json – copy the text and hand it to Claude Code.",
      copy: "Copy",
      copied: "Copied.",
      copyFailed: "Could not copy – please select the text by hand.",
      download: "Save as file",
    },
    heading: "Encounter",
    searchPlaceholder: "Search Pokémon...",
    noResults: "No matches",
    locked: "locked",
    clearSelection: "Clear selection",
    shinyToggle: "Mark as shiny (Shiny Clause)",
    bodyOfFusion: (head: string, route: string) => `Body of ${head} (${route})`,
    fusedWith: (body: string, route: string) => `Fused with ${body} (${route})`,
    caughtAsFusion: "Caught as a fusion",
    typeStatic: "Static",
    openOnly: "Open only",
    openOnlyTitle: "Show only routes without a complete entry",
    missingPlayer: "Encounter still missing for at least one player",
    clear: "Clear",
    clearConfirm: "Really delete this encounter?",
    nicknamePlaceholder: "Nickname",
    nicknameLabel: "Nickname",
    postgameShow: (n: number) => `Show post-game content (${n} areas)`,
    postgameHide: "Hide post-game content",
    allDone: "All routes done",
    showAll: "Show all",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} of ${total} routes completed (${percent}%)`,
  },
  links: {    free: {
      add: "Add Pokémon",
      title: "Add to the team",
      pokemonLabel: "Pokémon",
      playerLabel: "For",
      nicknameLabel: "Nickname",
      submit: "Add",
      added: (name: string) => `${name} added.`,
      removed: (name: string) => `${name} removed.`,
      remove: "Remove",
      removeConfirm: "Really remove?",
      removeYes: "Yes, remove",
      hint: "Creates its own entry behind the scenes. Does not count towards encounter progress.",
    },

    emptySoullink: "No SoulLinks yet – catch your first Pokémon in the Encounter tab.",
    emptyClassic: "No Pokémon yet – catch your first one in the Encounter tab.",
    markDead: "Mark as dead",
    clearTeam: "Clear team",
    clearTeamConfirm: "Empty all six team slots? The Pokémon themselves are kept.",
    dead: "Dead",
    deadBy: (name: string) => `lost by ${name}`,
    whoLost: "Who lost?",
    noAttribution: "No attribution",
    deathCausePlaceholder: "Cause of death (optional)",
    revive: "Revive",
    evolve: "Evolve",
    revert: "Devolve",
    staticTag: "static",
    rankSummary: (rang: number, summe: number) => `Rank #${rang} · BST ${summe}`,
    fusionSummary: (summe: number) => `Fusion · BST ${summe}`,
    fuse: "Fuse",
    fuseDialogTitle: "Fuse",
    fuseDialogHint:
      "Fuses this catch (head) with another catch of yours (body). Both stay separate catches - only the card shows them together from now on.",
    fuseDonorLabel: "Body",
    fuseNoCandidates: "No other caught Pokémon to fuse with.",
    fuseNoCustomSprite: "This combination has no hand-drawn sprite, only an auto-generated one.",
    fuseCdnUnreachable: "Sprite server unreachable.",
    fuseSubmit: "Fuse",
    fuseSuccess: "Fused.",
    swap: "Swap",
    swapTitle: "Swap head and body",
    unfuse: "Unfuse",
    unfuseConfirm: "Really unfuse?",
    fusionTag: "Fusion",
    bodyLabel: "Body",
    slotTakenBy: (routes: string) => `Taken by ${routes}`,
    slotTakenHint:
      "This group needs another slot while one player has more separate Pokémon in it than the other.",
    sortLabel: "Sort",
    sortDefault: "Encounter order",
    sortSumme: "BST",
    sortSummeMax: "Max BST",
    filterEvolvable: "Evolvable",
    filterEvolvableTitle: "Show only Pokémon with a level evolution available now",
    hideTeam: "Hide team",
    hideTeamTitle: "Hide Pokémon that are on the team",
    filterTypes: "Types",
    filterTypesTitle: "Filter by type",
    filterTypesClear: "Reset",
    noMatches: "No Pokémon match the filters",
    teamHeading: "Current Team",
    teamSlotLabel: (n: number) => `Team slot ${n}`,
    teamEmpty: "Empty",
    teamSelectPlaceholder: "— Empty —",
    addToTeam: "Add to team",
    replaceHint: "Replace which?",
  },
  pokedex: {
    heading: "Pokédex",
    searchPlaceholder: "Search Pokémon...",
    noResults: "No matches",
    filterAll: "All",
    filterAvailable: "Only available",
    filterLocked: "Only locked",
    filterLegendary: "Only legendaries",
      resetFilters: "Reset filters",
    columns: {
      id: "ID",
      name: "Name",
      types: "Types",
      kp: "HP",
      ang: "Atk.",
      vert: "Def.",
      spA: "Sp. Atk.",
      spV: "Sp. Def.",
      spez: "Special",
      init: "Speed",
      rang: "Rank",
      summe: "BST",
    },
    detail: {
      rank: "Rank",
      weaknesses: "Type weaknesses",
      stats: "Base stats",
      moves: "Moves (by level)",
      evolution: "Evolution",
      noEvolution: "No evolution",
      fusionHead: "Head",
      fusionBody: "Body",
      noMoveData: "No move data is available for this game.",
      evolvesFrom: "Evolves from",
      level: (n: number) => `Lv. ${n}`,
      movePower: "Power",
      moveAccuracy: "Accuracy",
      movePp: "PP",
      moveCategory: "Category",
      moveEffect: "Effect",
      moveChance: (n: number) => `${n}% chance`,
      moveToday: (value: string) => `today ${value}`,
      moveClass: { physical: "Physical", special: "Special", status: "Status" },
      moveDetails: "Toggle details",
      weight: "Weight",
      dexNo: "No.",
      forms: "Formes",
      formBase: "Normal Forme",
      openInAnalyze: "Open in Battle & Catch",
    },
  },
  errorPage: {
    title: "Something went wrong",
    hint: "This page could not be loaded. Trying again usually helps.",
    retry: "Try again",
  },
  levelcaps: {
    heading: "Journey",
    maxLevel: "Max Level",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} of ${total} battles completed (${percent}%)`,
    eliteFourMarker: "Elite Four starts here",
    empty: "No battles defined in this game pack.",
  },
  weaknesses: {
    heading: "Team Weaknesses",
    empty: "No team set up yet – pick up to 6 Pokémon for your team in the Pokémon tab.",
    attackType: "Attack type",
    weakHeader: "Weak",
    resistHeader: "Resists",
    criticalHint:
      "⚠ = critical weakness: this attack type hits at least half the team super-effectively with nobody resisting it.",
  },
  rules: {
    debugHeading: "Debug",
    heading: "Rules",
    edit: "Edit",
    save: "Save",
    cancel: "Cancel",
    loadDefault: "Load default",
    loadDefaultConfirm: "Replace the current rules with the default ruleset? (Applied only when you hit Save.)",
    settingsHeading: "Rule settings",
    playerNamesHeading: "Player names",
    playerNamesHint: "Replaces “Player 1”, “Player 2” … in this run.",
    presets: {
      label: "Ruleset",
      placeholder: "Load ruleset…",
      manage: "Rulesets",
      dialogTitle: "Rulesets",
      nameLabel: "Ruleset name",
      namePlaceholder: "e.g. SoulLink standard",
      saveCurrent: "Save",
      overwrite: "Overwrite",
      overwriteHint: (name: string) => `“${name}” already exists and will be replaced.`,
      existing: "Saved rulesets",
      none: "No rulesets saved yet.",
      scopeHint: "Rulesets are app-wide and available in every run.",
      editingHint: "Save the notes first.",
      delete: "Delete",
      deleteConfirm: "Really delete?",
      deleteYes: "Yes, delete",
      applyConfirm: (name: string) =>
        `Load “${name}”? This run’s rule settings and house rules will be overwritten.`,
      applied: (name: string) => `“${name}” loaded.`,
      saved: (name: string) => `“${name}” saved.`,
      deleted: (name: string) => `“${name}” deleted.`,
    },
    notesHeading: "House rules & notes",
    showNotes: "Show",
    hideNotes: "Hide",
    toggles: {
      freeTeam: {
        label: "Free team (no Nuzlocke)",
        description:
          "For an ordinary playthrough: the Team tab gets an \u201cadd a Pok\u00e9mon\u201d button instead of every member having to be entered against a route. The Encounter tab keeps working.",
      },
      customSpritesOnly: {
        label: "Hand-drawn sprites only",
        description:
          "Fusing shows a warning when the chosen pair has no hand-drawn sprite, only an auto-generated one. Never blocks the fusion.",
      },
      randomizerFuseEverything: {
        label: "Randomizer: Fuse everything",
        description:
          "For runs with the randomizer’s “Fuse everything” option: every wild Pokémon is already a fusion, so the Encounter tab shows the body field right away, without the checkbox. Left empty, the catch is not a fusion.",
      },
      speciesClausePerPlayer: {
        label: "Species Clause per player",
        description:
          "Each player has their own Species Clause: an encounter locks the family only for the player who had it – caught, fled or defeated alike.",
      },
      speciesClauseLinkedLocksAll: {
        label: "Caught links lock for everyone",
        description:
          "Only with “per player”: once a link forms because every player caught on the route, the family is used up for all players. Fled or defeated Pokémon still lock only their own player.",
      },
      wildFusionSplit: {
        label: "Wild fusions may be split",
        description:
          "A catch that was already a fusion when caught can be split back into two Pokémon, exactly as the game allows. Off = such a catch stays one unit forever. Self-made fusions are unaffected.",
      },
      fusionLocksBothFamilies: {
        label: "Wild fusions lock both families",
        description:
          "A wild-caught fusion locks the evolution families of both species, like two separate catches would. Off = only the head locks.",
      },
      blindflug: {
        label: "Blind flight",
        description:
          "Hides everything the app works out for you: type effectiveness everywhere, plus move lists and descriptions. The type chart itself stays. Applies to the whole run.",
      },
      debugMode: {
        label: "Debug: record encounter order",
        description:
          "Shows an export of the order routes were entered in on the Encounter tab – for correcting a game pack’s routes.json. The order is always recorded; this only reveals the export, and only on this device.",
      },
      speciesClause: {
        label: "Species Clause",
        description:
          "Warns when an evolution family has already been used in this run. Never blocks saving.",
      },
      shinyClause: {
        label: "Shiny Clause",
        description:
          "A catch marked shiny is exempt from the Species Clause (shinies may always be caught).",
      },
      staticsExemptFromClause: {
        label: "Statics exempt from the Species Clause",
        description:
          "No clause warning is shown on static locations (gifts, fossils, legendaries).",
      },
      nicknames: {
        label: "Nicknames",
        description:
          "Input field on the Encounter tab; cards on the Pokémon tab show the nickname.",
      },
      evolutionOverridesImpossible: {
        label: "Randomizer: impossible evolutions changed",
        description:
          "Replaced trade & co. evolutions from evolution-overrides.json (e.g. Kadabra → at level 37). Off = vanilla methods.",
      },
      evolutionOverridesEasier: {
        label: "Randomizer: evolutions made easier",
        description:
          "Lowered evolution levels from evolution-overrides.json (e.g. Dragonair → at level 40). Off = vanilla levels.",
      },
      evolutionOverridesTimeBased: {
        label: "Randomizer: time-based evolutions removed",
        description:
          "Day/night conditions dropped (Eevee → Espeon/Umbreon via Sun/Moon Stone, happiness and held-item evolutions without a time window). Off = original conditions.",
      },
      statics: {
        label: "Static encounters",
        description:
          "Shows static locations on the Encounter tab. Off = only regular routes are tracked.",
      },
    },
  },
  catchrate: {
    heading: "Catchrate",
    hint: "Select a Pokémon to calculate the catch chance.",
    lockWarning: "This Pokémon is locked by the Species Clause in this run.",
    lockWarningPlayer: "Locked by the Species Clause for this player.",
    caughtHeading: "Pokémon caught?",
    caughtNeedSelection: "Select a Pokémon above first.",
    caughtNoRoutes: "No open routes left.",
    selectRoute: "Pick a route …",
    caughtDone: (name: string, route: string) => `Recorded ${name} as caught on ${route}.`,
    confirmCatch: "Catch",
    conditionMet: "Condition met",
    addCard: "Add calculator",
    ballLabel: "Poké Ball",
    hpLabel: "HP (%)",
    levelLabel: "Wild Pokémon's level",
    ownLevelLabel: "Your Pokémon's level",
    statusLabel: "Status condition",
    turnLabel: "Battle turn",
    resultLabel: "Catch chance per throw",
    guaranteed: "Guaranteed catch",
    avgThrows: (n: string) => `Ø ${n} throws until caught`,
    details: (rate: number, ball: string, status: string) =>
      `Base catch rate ${rate} · Ball ${ball} · Status ${status}`,
    balls: {
      poke: "Poké Ball",
      great: "Great Ball",
      ultra: "Ultra Ball",
      master: "Master Ball",
      safari: "Safari Ball",
      net: "Net Ball",
      nest: "Nest Ball",
      dive: "Dive Ball",
      repeat: "Repeat Ball",
      timer: "Timer Ball",
      luxury: "Luxury Ball",
      premier: "Premier Ball",
      level: "Level Ball",
      lure: "Lure Ball",
      moon: "Moon Ball",
      friend: "Friend Ball",
      love: "Love Ball",
      fast: "Fast Ball",
      park: "Park Ball",
      quick: "Quick Ball",
      dusk: "Dusk Ball",
      heal: "Heal Ball",
      dream: "Dream Ball",
      heavy: "Heavy Ball",
      sport: "Sport Ball",
      cherish: "Cherish Ball",
      gender: "Gender Ball",
      boost: "Boost Ball",
      ability: "Ability Ball",
      virus: "Virus Ball",
      glitter: "Glitter Ball",
      perfect: "Perfect Ball",
      toxic: "Toxic Ball",
      spark: "Spark Ball",
      scorch: "Scorch Ball",
      frost: "Frost Ball",
      pure: "Pure Ball",
      status: "Status Ball",
      candy: "Candy Ball",
      rocket: "Rocket Ball",
      fusion: "Fusion Ball",
    },
    statusOptions: {
      none: "None",
      sleep: "Sleep",
      freeze: "Frozen",
      paralysis: "Paralysis",
      poison: "Poison",
      burn: "Burn",
    },
    ballNotes: {
      net: "Net Ball: 3× only applies against Water and Bug Pokémon (applied automatically based on type).",
      nest: "Nest Ball: bonus depends on the wild Pokémon's level (up to ×3.9 at level 1, no bonus from level 30).",
      dive: "Dive Ball: 3.5× only applies while surfing/diving/fishing – assumed here.",
      repeat: "Repeat Ball: 3× only applies if the species is already registered in the Pokédex – assumed here.",
      timer: "Timer Ball: bonus grows with the battle turn (max ×4 from turn 30).",
      level: "Level Ball: ×2 if your Pokémon's level is higher than the wild one's, ×4 at twice and ×8 at four times its level.",
      lure: "Lure Ball: ×3 only applies while fishing – assumed here.",
      moon: "Moon Ball: ×4 only for Moon Stone families (e.g. Nidoran, Clefairy) – assumed here.",
      friend: "Friend Ball: no catch bonus, sets friendship after the catch.",
      love: "Love Ball: ×8 only for the same species of the opposite gender – assumed here.",
      fast: "Fast Ball: ×4 only for flee-prone species – assumed here.",
      park: "Park Ball: ×1.5 during the Bug-Catching Contest.",
      quick: "Quick Ball: ×4 only on turn 1, ×1 afterwards.",
      dusk: "Dusk Ball: ×3.5 only at night or in caves – assumed here.",
      heal: "Heal Ball: no catch bonus, fully heals the caught Pokémon.",
      dream: "Dream Ball: a guaranteed catch in Gen 5 (Dream World only).",
      heavy: "Heavy Ball: shifts the catch rate by the target's weight (−20 below 102 kg, up to +40 from 410 kg) instead of multiplying it.",
      sport: "Sport Ball: no catch bonus, only available during the Bug-Catching Contest.",
    },
    ballNotesInfiniteFusion: {
      nest: "Nest Ball: bonus only up to level 30, (41 − level) ÷ 10 – so ×4 at level 1.",
      timer: "Timer Ball: ×1 on turn 1, then +0.3 per turn, up to ×4.",
      quick: "Quick Ball: ×5 on turn 1 only, ×1 afterwards.",
      fast: "Fast Ball: ×4 against Pokémon with base Speed 100 or more (applied automatically).",
      heavy: "Heavy Ball: −20 below 204.8 kg, +20 from 204.8 kg, +30 from 307.2 kg, +40 from 409.6 kg. A fusion weighs the average of both Pokémon.",
      sport: "Sport Ball: ×1.5.",
      dream: "Dream Ball: puts the Pokémon to sleep when thrown, so the sleep bonus ×2.5 applies. Not a guaranteed catch.",
      cherish: "Cherish Ball: no catch bonus.",
      gender: "Gender Ball: no catch bonus, swaps the Pokémon's gender once caught.",
      boost: "Boost Ball: ×0.8, the Pokémon counts as traded once caught.",
      ability: "Ability Ball: ×0.6, the Pokémon gets its hidden ability.",
      virus: "Virus Ball: ×0.4, the Pokémon gets Pokérus.",
      glitter: "Glitter Ball: ×0.2, the Pokémon becomes shiny.",
      perfect: "Perfect Ball: ×0.1, one or two random IVs are set to 31.",
      toxic: "Toxic Ball: poisons the Pokémon when thrown, so the status bonus ×1.5 applies.",
      spark: "Spark Ball: paralyzes the Pokémon when thrown, so the status bonus ×1.5 applies.",
      scorch: "Scorch Ball: burns the Pokémon when thrown, so the status bonus ×1.5 applies.",
      frost: "Frost Ball: freezes the Pokémon when thrown, so the status bonus ×2.5 applies.",
      pure: "Pure Ball: meant to give ×3.5 without a status condition. The game's code never applies it, so ×1 here.",
      status: "Status Ball: meant to give ×2.5 with a status condition. The game's code always applies it, so always ×2.5 here.",
      candy: "Candy Ball: ×0.8, the Pokémon is 5 levels higher once caught.",
      rocket: "Rocket Ball: no catch bonus.",
      fusion: "Fusion Ball: ×3 against fusions (applied automatically once a body is picked).",
    },
    lastBallLabel: "Last ball of its kind in the bag",
    lastBallHint: "Then a critical capture can happen: a single shake check instead of four.",
    dexOwnedLabel: "Species caught in the Pokédex",
    dexOwnedOptions: ["up to 300", "301–450", "451–600", "over 600"],
    caughtNoFusion:
      "Fusions can't be recorded here – record the head as an encounter, then fuse it on the Team tab.",
  },
  typen: {
    heading: "Type Effectiveness",
    battleHeading: "Battle",
    analyzeHeading: "Battle & Catch",
    viewWild: "Wild",
    viewTrainer: "Trainer",
    battleHint: "Pick an opponent to see its types, moves, and the team matchup.",
    levelLabel: "Level",
    moveTypes: "Attack types",
    damagingOnly: "damaging moves only",
    bodyPickerLabel: "Body (optional, for fusions)",
    noMoveData: "No move data is available for this game.",
    teamMatchup: "How well does your team hold up?",
    explosionWarn: (name: string, lvl: number) => `⚠ Can use ${name} from Lv ${lvl}!`,
    hint: "Select a Pokémon to see its weaknesses and resistances.",
    weak4: "4× Weak to",
    weak2: "2× Weak to",
    resist2: "0.5× Resists",
    resist4: "0.25× Strongly resists",
    immune: "0× Immune to",
    attack: "Attack",
    defense: "Defense",
    statsHeading: "Stats",
    defenderWeakHeading: "Weaknesses as defender",
    teamStrengthHeading: "Strengths against my team",
    matrixHeading: "Type matrix",
    legendOffense: "Green = you hit hard · Red = you hit weakly · Black = no effect",
    legendDefense: "Green = your team takes little · Red = your team takes a lot · Black = immune",
    addCard: "Add opponent",
  },
  tms: {
    heading: "TM compatibility",
    searchPlaceholder: "Search move …",
    noResults: "No move found",
    hint: "Pick a move to see which of your Pokémon can officially learn it.",
    caveat: "Source: official game data (original compatibility) – not the randomized ROM assignment.",
    notLearnable: "This move is not a TM/HM or tutor move in the original game.",
    empty: "No living Pokémon caught yet.",
    teamHeading: "Team",
    bankHeading: "Bench",
    method: { tm: "TM", hm: "HM", tutor: "Tutor", level: "Lv." },
    noMoveData: "No TM or move data is available for this game.",
  },
  overview: {
    heading: "Overview",
    progress: "Progress",
    encounterProgress: "Encounters",
    battleProgress: "Journey",
    badgesHeading: "Badges",
    badgesEmpty: "No badges are configured for this game.",
    dashboard: "Dashboard",
    coverage: "Team coverage",
    offensiveGaps: "Offensive gaps",
    offensiveGapsInfo: (level: number) =>
      `Only counts attack types the team has actually learned by the last cleared level cap (level ${level}).`,
    noGaps: "No gaps – the team hits every type super-effectively.",
    offensiveHint: "Types no team member hits super-effectively.",
    memorial: "Memorial",
    memorialEmpty: "No losses yet. Stay careful.",
    deathPoint: "Time of death",
    deathPointNone: "No point recorded",
    deathPointUnset: "Set when it died …",
    totalDeaths: "Total deaths",
    teamBst: "Team BST (now → max)",
    levelCap: "Level cap (now → next)",
    deathTallyHeading: "Responsible for deaths",
    deathTallyUnattributed: (n: number) => `+${n} unattributed`,
    player: "Player",
    team: "Team",
    caught: "Caught",
    bankBstCol: "Bench BST",
    bankBstColHint: "BST of caught, living Pokémon not on the team (now → fully evolved)",
    teamBstCol: "Team BST",
    teamBstColHint: "BST of the team's Pokémon (now → fully evolved)",
    emptyRun: "No Pokémon caught yet.",
    emptyRunHint: "As soon as you catch something on the Encounter tab, BST, coverage and stats show up here.",
    bankBst: "Bank BST (now → max)",
    caughtTotal: "Caught",
  },
  actions: {
    unknownPokemon: (id: number) => `Unknown Pokémon id: ${id}`,
    unknownRoute: (id: number) => `Unknown route id: ${id}`,
    speciesLocked: (pokemonName: string, playerLabel: string, routeName: string) =>
      `${pokemonName} is locked by the Species Clause (already used by ${playerLabel} on ${routeName}).`,
    playerNotInRun: "This run has no such player.",
    encounterNotFound: (id: number) => `Encounter #${id} not found.`,
    soulLinkNotFound: (id: number) => `SoulLink #${id} not found.`,
    runNotFound: "Run not found - the link is invalid or the run was deleted.",
    notOnPublicInstance: "This is not available on the public site.",
    deadCannotEvolve: "A dead Pokémon can't be evolved.",
    deadCannotRevert: "A dead Pokémon can't be devolved.",
    invalidEvolutionTarget: "Invalid evolution target.",
    invalidFormTarget: "Invalid forme for this Pokémon.",
    evolutionFamilyMismatch: "Internal error: family mismatch in evolutions.json.",
    noPreEvolution: "There's no pre-evolution.",
    unknownLevelCap: (id: number) => `Unknown level cap: ${id}`,
    presetNotFound: (id: number) => `Unknown ruleset: ${id}`,
    nameRequired: "Name must not be empty.",
    backupInvalid: "Invalid or corrupted backup file.",
    backupEmpty: "The backup file contains no runs.",
    invalidTeamSlot: "Invalid team slot.",
    fusionSameEncounter: "Head and body must be two different catches.",
    fusionDifferentPlayer: "Head and body must belong to the same player.",
    fusionNotCaught: "Both catches must be alive and caught to fuse them.",
    fusionChain: "One of the two is already part of another fusion.",
    fusionNotFound: "That card isn't a fusion.",
    fusionDead: "A dead fusion can't be changed.",
    fusionSplitDisabled: "This run has the \"wild fusions may be split\" rule switched off.",
    unexpected: "Unexpected error - the action was not applied. See the server log for details.",
  },
};

const fr: Dictionary = {
  header: {
    titleShort: "SoulLink Tracker",
    titleFull: "SoulLink Nuzlocke Tracker",
  },
  nav: {
    tracker: "Rencontres",
    links: "Équipe",
    pokedex: "Pokédex",
    typen: "Combat & Capture",
    weaknesses: "Faiblesses de l'équipe",
    overview: "Aperçu",
    tms: "CT",
    catchrate: "Taux de capture",
    levelcaps: "Aventure",
    rules: "Règles",
  },
  theme: {
    toggleNeutral: "Changer de thème",
    toDark: "Passer au thème sombre",
    toLight: "Passer au thème clair",
  },
  language: {
    switchLabel: "Changer de langue",
  },
  dialog: {
    ok: "OK",
    cancel: "Annuler",
    confirm: "Confirmer",
    saving: "Enregistrement…",
    saved: "Enregistré",
  },
  menu: {
    copyRunLink: "Copier le lien du run",
    runLinkCopied: "Lien copié. Toute personne qui l'a peut voir et modifier le run.",
    runLinkManual: (url: string) => `Lien du run - toute personne qui l'a peut voir et modifier le run :\n\n${url}`,
    label: "Menu",
    language: "Langue",
    version: "Version",
    credits: "Crédits",
    creditsSprites: "Sprites de fusion",
    creditsSpritesSource: "Communauté de sprites d'Infinite Fusion (lien direct)",
    creditsData: "Routes et lieux",
    creditsMechanics: "Mécanique des Balls",
  },
  install: {
    menuLabel: "Installer l'application",
    iosTitle: "Installer l'application",
    iosHint:
      "Dans Safari, appuyez sur « Partager » en bas, puis sur « Sur l'écran d'accueil ». " +
      "Remarque : sous iOS, l'application installée dispose de son propre stockage – le thème, " +
      "l'ordre des onglets et la langue reviennent une fois aux valeurs par défaut.",
    manualHint:
      "Votre navigateur ne propose pas d'installation directe. Ouvrez son menu et choisissez " +
      "« Ajouter à l'écran d'accueil » ou « Installer ».",
  },
  blindflug: {
    name: "Vol à l’aveugle",
    hidden: "Masqué en vol à l’aveugle",
    enable: "Activer le vol à l’aveugle",
    disable: "Désactiver le vol à l’aveugle",
  },
  appearance: {
    label: "Affichage",
    title: "Affichage",
    sizeLabel: "Taille de l’interface",
    smaller: "Petit",
    normal: "Normal",
    larger: "Grand",
    preview: "Voici à quoi ressemble le texte à cette taille.",
    previewSmall: "Et voici les petites mentions en dessous.",
    hint: "Ne vaut que pour cet appareil.",
    reset: "Réinitialiser",
    done: "Terminé",
  },
  tabOrder: {
    label: "Organiser les onglets",
    title: "Organiser les onglets",
    moveUp: "Monter",
    moveDown: "Descendre",
    reset: "Réinitialiser",
    done: "Terminé",
    hide: "Masquer l'onglet",
    show: "Afficher l'onglet",
    visibilityHint: "Décoche pour masquer un onglet.",
  },
  backup: {
    menuLabel: "Sauvegarde & Import",
    backupRun: "Sauvegarder le run actuel",
    backupAll: "Sauvegarder tous les runs",
    import: "Importer une sauvegarde …",
    importSuccess: (n: number) => `${n} run(s) importé(s).`,
    importNewerVersion:
      "Cette sauvegarde a été créée avec une version plus récente de l'application. Certaines données (par ex. les fusions) pourraient ne pas être importées.",
    noRun: "Aucun run sélectionné.",
    importTitle: "Importer une sauvegarde",
    importIntro:
      "Donnez un nom à chaque run de la sauvegarde avant qu'il ne soit ajouté comme nouveau run.",
    importNameLabel: (i: number, total: number) =>
      total > 1 ? `Nom du run ${i + 1} sur ${total}` : "Nom du nouveau run",
    importOriginalName: (name: string) => `Dans la sauvegarde : « ${name} »`,
    importUseBackupName: "Utiliser le nom de la sauvegarde",
    importNameCollision: "Un run porte déjà ce nom.",
    importConfirm: "Importer",
    cancel: "Annuler",
  },
  landing: {
    title: "Nuzlocke Tracker",
    empty: "Aucun run n'est encore ouvert sur cet appareil.",
    hint: "Crée un nouveau run. Pour rejoindre un SoulLink, ouvre simplement le lien que tes coéquipiers t'ont envoyé.",
    create: "Créer un nouveau run",
    linkNote: "Chaque run a son propre lien secret. Toute personne qui le connaît peut voir et modifier le run - ne le partage qu'avec tes coéquipiers.",
  },
  runSwitcher: {
    newRun: "+ Nouveau run",
    rename: "Renommer",
    renameTitle: "Renommer le run",
    renameLabel: (name: string) => `Renommer le run « ${name} »`,
    save: "Enregistrer",
    deleteButton: "Supprimer le run",
    deleteLabel: (name: string) => `Supprimer le run « ${name} »`,
    newRunTitle: "Nouveau run",
    nameLabel: "Nom",
    namePlaceholder: "p. ex. Playthrough 2",
    gameLabel: "Jeu",
    modeLabel: "Mode",
    modeSoullink: "SoulLink",
    modeClassic: "Solo",
    cancel: "Annuler",
    create: "Créer",
    confirmDelete1: (name: string) =>
      `Vraiment supprimer le run « ${name} » ? Toutes les rencontres, tous les liens et la progression des level caps de ce run seront définitivement perdus.`,
    soloSuffix: " (Solo)",
    playersLabel: "Joueurs",
    playersSuffix: (n: number) => ` (${n} joueurs)`,
  },
  player: {
    PLAYER1: "Joueur 1",
    PLAYER2: "Joueur 2",
    PLAYER3: "Joueur 3",
    PLAYER4: "Joueur 4",
  },
  status: {
    CAUGHT: "Capturé",
    KILLED: "Tué",
    FLED: "Enfui",
  },
  tracker: {
    custom: {
      manage: "Routes personnalisées",
      title: "Routes personnalisées",
      nameLabel: "Nom",
      namePlaceholder: "p. ex. Parc Safari (suppl.)",
      typeLabel: "Type",
      typeRoute: "Route",
      typeStatic: "Statique",
      positionLabel: "Insérer après",
      positionTop: "(tout en haut)",
      add: "Ajouter",
      existing: "Routes personnalisées de ce run",
      none: "Aucune route personnalisée.",
      hint: "Compte dans la barre de progression et ne vaut que pour ce run.",
      delete: "Supprimer",
      deleteConfirm: (n: number) =>
        n === 0
          ? "Vraiment supprimer ?"
          : `Vraiment supprimer ? ${n} rencontres seront supprimées avec.`,
      deleteYes: "Oui, supprimer",
      added: (name: string) => `« ${name} » ajouté.`,
      deleted: (name: string) => `« ${name} » supprimé.`,
    },
    debug: {
      export: "Exporter l’ordre",
      title: "Ordre des rencontres",
      hint: "Pour corriger le routes.json du pack – copiez le texte et donnez-le à Claude Code.",
      copy: "Copier",
      copied: "Copié.",
      copyFailed: "Copie impossible – sélectionnez le texte à la main.",
      download: "Enregistrer comme fichier",
    },
    heading: "Rencontres",
    searchPlaceholder: "Rechercher un Pokémon...",
    noResults: "Aucun résultat",
    locked: "bloqué",
    clearSelection: "Effacer la sélection",
    shinyToggle: "Marquer comme chromatique (Clause Chromatique)",
    bodyOfFusion: (head: string, route: string) => `Corps de ${head} (${route})`,
    fusedWith: (body: string, route: string) => `Fusionné avec ${body} (${route})`,
    caughtAsFusion: "Capturé en fusion",
    typeStatic: "Statique",
    openOnly: "Ouvertes",
    openOnlyTitle: "N'afficher que les routes sans entrée complète",
    missingPlayer: "Rencontre encore manquante pour au moins un joueur",
    clear: "Vider",
    clearConfirm: "Vraiment supprimer cette rencontre ?",
    nicknamePlaceholder: "Surnom",
    nicknameLabel: "Surnom",
    postgameShow: (n: number) => `Afficher le contenu post-ligue (${n} zones)`,
    postgameHide: "Masquer le contenu post-ligue",
    allDone: "Toutes les routes sont terminées",
    showAll: "Tout afficher",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} routes sur ${total} terminées (${percent}%)`,
  },
  links: {    free: {
      add: "Ajouter un Pokémon",
      title: "Ajouter à l’équipe",
      pokemonLabel: "Pokémon",
      playerLabel: "Pour",
      nicknameLabel: "Surnom",
      submit: "Ajouter",
      added: (name: string) => `${name} ajouté.`,
      removed: (name: string) => `${name} retiré.`,
      remove: "Retirer",
      removeConfirm: "Vraiment retirer ?",
      removeYes: "Oui, retirer",
      hint: "Crée sa propre entrée en arrière-plan. Ne compte pas dans la progression des rencontres.",
    },

    emptySoullink:
      "Pas encore de SoulLinks – capture ton premier Pokémon dans l'onglet Rencontres.",
    emptyClassic: "Pas encore de Pokémon – capture le premier dans l'onglet Rencontres.",
    markDead: "Marquer comme mort",
    clearTeam: "Vider l'équipe",
    clearTeamConfirm: "Vider les six emplacements ? Les Pokémon eux-mêmes sont conservés.",
    dead: "Mort",
    deadBy: (name: string) => `perdu par ${name}`,
    whoLost: "Qui a perdu ?",
    noAttribution: "Sans attribution",
    deathCausePlaceholder: "Cause de la mort (optionnel)",
    revive: "Ranimer",
    evolve: "Faire évoluer",
    revert: "Annuler l'évolution",
    staticTag: "statique",
    rankSummary: (rang: number, summe: number) => `Rang #${rang} · BST ${summe}`,
    fusionSummary: (summe: number) => `Fusion · BST ${summe}`,
    fuse: "Fusionner",
    fuseDialogTitle: "Fusionner",
    fuseDialogHint:
      "Fusionne cette capture (tête) avec une autre de vos captures (corps). Les deux restent des captures séparées - seule la carte les montre désormais ensemble.",
    fuseDonorLabel: "Corps",
    fuseNoCandidates: "Aucune autre capture à fusionner.",
    fuseNoCustomSprite: "Cette combinaison n'a pas de sprite dessiné à la main, seulement un généré automatiquement.",
    fuseCdnUnreachable: "Serveur de sprites injoignable.",
    fuseSubmit: "Fusionner",
    fuseSuccess: "Fusionné.",
    swap: "Échanger",
    swapTitle: "Échanger tête et corps",
    unfuse: "Défusionner",
    unfuseConfirm: "Vraiment défusionner ?",
    fusionTag: "Fusion",
    bodyLabel: "Corps",
    slotTakenBy: (routes: string) => `Occupé par ${routes}`,
    slotTakenHint:
      "Ce groupe a besoin d'une place de plus tant qu'un joueur y a plus de Pokémon séparés que l'autre.",
    sortLabel: "Tri",
    sortDefault: "Ordre des rencontres",
    sortSumme: "BST",
    sortSummeMax: "Max BST",
    filterEvolvable: "Évolution possible",
    filterEvolvableTitle:
      "N'afficher que les Pokémon avec une évolution par niveau possible maintenant",
    hideTeam: "Masquer l'équipe",
    hideTeamTitle: "Masquer les Pokémon de l'équipe",
    filterTypes: "Types",
    filterTypesTitle: "Filtrer par type",
    filterTypesClear: "Réinitialiser",
    noMatches: "Aucun Pokémon ne correspond aux filtres",
    teamHeading: "Équipe actuelle",
    teamSlotLabel: (n: number) => `Place ${n}`,
    teamEmpty: "Vide",
    teamSelectPlaceholder: "— Vide —",
    addToTeam: "Dans l'équipe",
    replaceHint: "Remplacer qui ?",
  },
  pokedex: {
    heading: "Pokédex",
    searchPlaceholder: "Rechercher un Pokémon...",
    noResults: "Aucun résultat",
    filterAll: "Tous",
    filterAvailable: "Disponibles",
    filterLocked: "Bloqués",
    filterLegendary: "Légendaires uniquement",
      resetFilters: "Réinitialiser les filtres",
    columns: {
      id: "ID",
      name: "Nom",
      types: "Types",
      kp: "PV",
      ang: "Att.",
      vert: "Déf.",
      spA: "Att. Spé.",
      spV: "Déf. Spé.",
      spez: "Spécial",
      init: "Vit.",
      rang: "Rang",
      summe: "BST",
    },
    detail: {
      rank: "Rang",
      weaknesses: "Faiblesses de type",
      stats: "Statistiques",
      moves: "Attaques (par niveau)",
      evolution: "Évolution",
      noEvolution: "Aucune évolution",
      fusionHead: "Tête",
      fusionBody: "Corps",
      noMoveData: "Aucune donnée d'attaque n'est disponible pour ce jeu.",
      evolvesFrom: "Évolue de",
      level: (n: number) => `N. ${n}`,
      movePower: "Puissance",
      moveAccuracy: "Précision",
      movePp: "PP",
      moveCategory: "Catégorie",
      moveEffect: "Effet",
      moveChance: (n: number) => `${n} % de chance`,
      moveToday: (value: string) => `auj. ${value}`,
      moveClass: { physical: "Physique", special: "Spéciale", status: "Statut" },
      moveDetails: "Afficher/masquer les détails",
      weight: "Poids",
      dexNo: "N°",
      forms: "Formes",
      formBase: "Forme normale",
      openInAnalyze: "Ouvrir dans Combat & Capture",
    },
  },
  errorPage: {
    title: "Une erreur est survenue",
    hint: "Cette page n'a pas pu être chargée. Réessayer suffit généralement.",
    retry: "Réessayer",
  },
  levelcaps: {
    heading: "Aventure",
    maxLevel: "Niveau max",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} combats sur ${total} terminés (${percent}%)`,
    eliteFourMarker: "Le Conseil 4 commence ici",
    empty: "Aucun combat défini dans ce pack de jeu.",
  },
  weaknesses: {
    heading: "Faiblesses de l'équipe",
    empty: "Aucune équipe – choisis jusqu'à 6 Pokémon dans l'onglet Pokémon.",
    attackType: "Type d'attaque",
    weakHeader: "Faible",
    resistHeader: "Résiste",
    criticalHint:
      "⚠ = faiblesse critique : ce type d'attaque touche au moins la moitié de l'équipe de façon super efficace sans que personne n'y résiste.",
  },
  rules: {
    debugHeading: "Débogage",
    heading: "Règles",
    edit: "Modifier",
    save: "Enregistrer",
    cancel: "Annuler",
    loadDefault: "Charger le modèle",
    loadDefaultConfirm: "Remplacer les règles actuelles par le règlement par défaut ? (Appliqué seulement en cliquant sur Enregistrer.)",
    settingsHeading: "Paramètres des règles",
    playerNamesHeading: "Noms des joueurs",
    playerNamesHint: "Remplace « Joueur 1 », « Joueur 2 » … dans ce run.",
    presets: {
      label: "Règlement",
      placeholder: "Charger un règlement…",
      manage: "Règlements",
      dialogTitle: "Règlements",
      nameLabel: "Nom du règlement",
      namePlaceholder: "p. ex. SoulLink standard",
      saveCurrent: "Enregistrer",
      overwrite: "Remplacer",
      overwriteHint: (name: string) => `« ${name} » existe déjà et sera remplacé.`,
      existing: "Règlements enregistrés",
      none: "Aucun règlement enregistré.",
      scopeHint: "Les règlements valent pour toute l’application et sont proposés dans chaque run.",
      editingHint: "Enregistrez d’abord les notes.",
      delete: "Supprimer",
      deleteConfirm: "Vraiment supprimer ?",
      deleteYes: "Oui, supprimer",
      applyConfirm: (name: string) =>
        `Charger « ${name} » ? Les paramètres de règles et les règles maison de ce run seront remplacés.`,
      applied: (name: string) => `« ${name} » chargé.`,
      saved: (name: string) => `« ${name} » enregistré.`,
      deleted: (name: string) => `« ${name} » supprimé.`,
    },
    notesHeading: "Règles maison & notes",
    showNotes: "Afficher",
    hideNotes: "Masquer",
    toggles: {
      freeTeam: {
        label: "Équipe libre (sans Nuzlocke)",
        description:
          "Pour une partie normale : l’onglet Équipe reçoit un bouton « ajouter un Pokémon » au lieu de devoir saisir chaque membre via une route. L’onglet Rencontres reste utilisable.",
      },
      customSpritesOnly: {
        label: "Sprites dessinés à la main uniquement",
        description:
          "La fusion affiche un avertissement quand la paire choisie n’a pas de sprite dessiné à la main, seulement un généré automatiquement. Ne bloque jamais la fusion.",
      },
      randomizerFuseEverything: {
        label: "Randomizer : tout fusionner",
        description:
          "Pour les parties avec l’option « Fuse everything » du randomizer : chaque Pokémon sauvage est déjà une fusion, l’onglet Rencontres affiche donc directement le champ du corps, sans case à cocher. Laissé vide, la capture n’est pas une fusion.",
      },
      speciesClausePerPlayer: {
        label: "Species Clause par joueur",
        description:
          "Chaque joueur a sa propre Species Clause : une rencontre bloque la famille uniquement pour le joueur qui l'a eue – capturée, en fuite ou vaincue.",
      },
      speciesClauseLinkedLocksAll: {
        label: "Les liens capturés bloquent pour tous",
        description:
          "Uniquement avec « par joueur » : dès qu'un lien se forme parce que tous les joueurs ont capturé sur la route, la famille est épuisée pour tous. Les Pokémon en fuite ou vaincus ne bloquent toujours que leur propre joueur.",
      },
      wildFusionSplit: {
        label: "Les fusions sauvages peuvent être séparées",
        description:
          "Une capture déjà fusionnée peut être séparée en deux Pokémon, comme dans le jeu. Désactivé = cette capture reste une seule unité. Les fusions faites soi-même ne sont pas concernées.",
      },
      fusionLocksBothFamilies: {
        label: "Les fusions sauvages bloquent les deux familles",
        description:
          "Une fusion capturée à l’état sauvage bloque les familles d’évolution des deux espèces, comme deux captures distinctes. Désactivé = seule la tête bloque.",
      },
      blindflug: {
        label: "Vol à l’aveugle",
        description:
          "Masque tout ce que l’application calcule pour vous : l’efficacité des types partout, ainsi que les listes et descriptions d’attaques. La table des types reste. Vaut pour tout le run.",
      },
      debugMode: {
        label: "Débogage : enregistrer l’ordre des rencontres",
        description:
          "Affiche dans l’onglet Rencontres un export de l’ordre dans lequel les routes ont été saisies – pour corriger le routes.json d’un pack. L’ordre est toujours enregistré ; ceci ne fait qu’afficher l’export, et seulement sur cet appareil.",
      },
      speciesClause: {
        label: "Species Clause",
        description:
          "Avertit quand une famille d'évolution a déjà été utilisée dans ce run. Ne bloque jamais l'enregistrement.",
      },
      shinyClause: {
        label: "Clause Chromatique",
        description:
          "Une capture marquée chromatique est exemptée de la clause d'espèce (les chromatiques peuvent toujours être capturés).",
      },
      staticsExemptFromClause: {
        label: "Statiques exemptés de la Species Clause",
        description:
          "Aucun avertissement sur les lieux statiques (cadeaux, fossiles, légendaires).",
      },
      nicknames: {
        label: "Surnoms",
        description:
          "Champ de saisie dans l'onglet Rencontres ; les cartes de l'onglet Pokémon affichent le surnom.",
      },
      evolutionOverridesImpossible: {
        label: "Randomizer : évolutions impossibles modifiées",
        description:
          "Évolutions par échange etc. remplacées selon evolution-overrides.json (p. ex. Kadabra → au niveau 37). Désactivé = méthodes d'origine.",
      },
      evolutionOverridesEasier: {
        label: "Randomizer : évolutions facilitées",
        description:
          "Niveaux d'évolution abaissés selon evolution-overrides.json (p. ex. Draco → au niveau 40). Désactivé = niveaux d'origine.",
      },
      evolutionOverridesTimeBased: {
        label: "Randomizer : évolutions temporelles supprimées",
        description:
          "Conditions jour/nuit retirées (Évoli → Mentali/Noctali via Pierre Soleil/Lune). Désactivé = conditions d'origine.",
      },
      statics: {
        label: "Rencontres statiques",
        description:
          "Affiche les lieux statiques dans l'onglet Rencontres. Désactivé = seules les routes normales sont suivies.",
      },
    },
  },
  catchrate: {
    heading: "Taux de capture",
    hint: "Choisis un Pokémon pour calculer la chance de capture.",
    lockWarning: "Ce Pokémon est bloqué par la Species Clause dans ce run.",
    lockWarningPlayer: "Bloqué par la Species Clause pour ce joueur.",
    caughtHeading: "Pokémon capturé ?",
    caughtNeedSelection: "Choisis d'abord un Pokémon ci-dessus.",
    caughtNoRoutes: "Plus de routes ouvertes.",
    selectRoute: "Choisir une route …",
    caughtDone: (name: string, route: string) => `${name} enregistré comme capturé sur ${route}.`,
    confirmCatch: "Attraper",
    conditionMet: "Condition remplie",
    addCard: "Ajouter une carte",
    ballLabel: "Poké Ball",
    hpLabel: "PV (%)",
    levelLabel: "Niveau du Pokémon sauvage",
    ownLevelLabel: "Niveau de ton Pokémon",
    statusLabel: "Altération de statut",
    turnLabel: "Tour de combat",
    resultLabel: "Chance de capture par lancer",
    guaranteed: "Capture garantie",
    avgThrows: (n: string) => `Ø ${n} lancers avant la capture`,
    details: (rate: number, ball: string, status: string) =>
      `Taux de base ${rate} · Ball ${ball} · Statut ${status}`,
    balls: {
      poke: "Poké Ball",
      great: "Super Ball",
      ultra: "Hyper Ball",
      master: "Master Ball",
      safari: "Safari Ball",
      net: "Filet Ball",
      nest: "Faiblo Ball",
      dive: "Scuba Ball",
      repeat: "Bis Ball",
      timer: "Chrono Ball",
      luxury: "Luxe Ball",
      premier: "Honor Ball",
      level: "Niveau Ball",
      lure: "Appât Ball",
      moon: "Lune Ball",
      friend: "Copain Ball",
      love: "Love Ball",
      fast: "Speed Ball",
      park: "Parc Ball",
      quick: "Rapide Ball",
      dusk: "Sombre Ball",
      heal: "Ball Soin",
      dream: "Rêve Ball",
      heavy: "Masse Ball",
      sport: "Compét’Ball",
      cherish: "Mémoire Ball",
      gender: "Gender Ball",
      boost: "Boost Ball",
      ability: "Ability Ball",
      virus: "Virus Ball",
      glitter: "Glitter Ball",
      perfect: "Perfect Ball",
      toxic: "Toxic Ball",
      spark: "Spark Ball",
      scorch: "Scorch Ball",
      frost: "Frost Ball",
      pure: "Pure Ball",
      status: "Status Ball",
      candy: "Candy Ball",
      rocket: "Rocket Ball",
      fusion: "Fusion Ball",
    },
    statusOptions: {
      none: "Aucune",
      sleep: "Sommeil",
      freeze: "Gel",
      paralysis: "Paralysie",
      poison: "Poison",
      burn: "Brûlure",
    },
    ballNotes: {
      net: "Filet Ball : ×3 seulement contre les Pokémon Eau et Insecte (appliqué automatiquement selon le type).",
      nest: "Faiblo Ball : le bonus dépend du niveau du Pokémon sauvage (jusqu'à ×3,9 au niveau 1, aucun bonus à partir du niveau 30).",
      dive: "Scuba Ball : ×3,5 seulement en surfant/plongeant/pêchant – supposé ici.",
      repeat: "Bis Ball : ×3 seulement si l'espèce est déjà enregistrée dans le Pokédex – supposé ici.",
      timer: "Chrono Ball : le bonus augmente avec le tour de combat (max ×4 à partir du tour 30).",
      level: "Niveau Ball : ×2 si ton Pokémon a un niveau plus élevé que le Pokémon sauvage, ×4 à partir du double et ×8 à partir du quadruple.",
      lure: "Appât Ball : ×3 seulement en pêchant – supposé ici.",
      moon: "Lune Ball : ×4 seulement pour les familles Pierre Lune (p. ex. Nidoran, Mélofée) – supposé ici.",
      friend: "Copain Ball : aucun bonus de capture, augmente le bonheur après la capture.",
      love: "Love Ball : ×8 seulement pour la même espèce de sexe opposé – supposé ici.",
      fast: "Speed Ball : ×4 seulement pour les espèces fuyardes – supposé ici.",
      park: "Parc Ball : ×1,5 pendant le Concours de Capture.",
      quick: "Rapide Ball : ×4 seulement au tour 1, ×1 ensuite.",
      dusk: "Sombre Ball : ×3,5 seulement la nuit ou dans les grottes – supposé ici.",
      heal: "Ball Soin : aucun bonus de capture, soigne le Pokémon capturé.",
      dream: "Rêve Ball : capture garantie en Gen 5 (Monde des Rêves uniquement).",
      heavy: "Masse Ball : décale le taux de capture selon le poids (−20 sous 102 kg, jusqu’à +40 dès 410 kg) au lieu de le multiplier.",
      sport: "Compét’Ball : aucun bonus de capture, uniquement pendant le Concours Attrap’Insecte.",
    },
    ballNotesInfiniteFusion: {
      nest: "Faiblo Ball : bonus uniquement jusqu'au niveau 30, (41 − niveau) ÷ 10 – donc ×4 au niveau 1.",
      timer: "Chrono Ball : ×1 au tour 1, puis +0,3 par tour, jusqu'à ×4.",
      quick: "Rapide Ball : ×5 seulement au tour 1, ×1 ensuite.",
      fast: "Speed Ball : ×4 contre les Pokémon avec une Vitesse de base d'au moins 100 (appliqué automatiquement).",
      heavy: "Masse Ball : −20 sous 204,8 kg, +20 dès 204,8 kg, +30 dès 307,2 kg, +40 dès 409,6 kg. Une fusion pèse la moyenne des deux Pokémon.",
      sport: "Compét’Ball : ×1,5.",
      dream: "Rêve Ball : endort le Pokémon au lancer, le bonus Sommeil ×2,5 s'applique donc. Capture non garantie.",
      cherish: "Mémoire Ball : aucun bonus de capture.",
      gender: "Gender Ball : aucun bonus de capture, inverse le sexe du Pokémon après la capture.",
      boost: "Boost Ball : ×0,8, le Pokémon compte comme échangé après la capture.",
      ability: "Ability Ball : ×0,6, le Pokémon obtient son talent caché.",
      virus: "Virus Ball : ×0,4, le Pokémon attrape le Pokérus.",
      glitter: "Glitter Ball : ×0,2, le Pokémon devient chromatique.",
      perfect: "Perfect Ball : ×0,1, un ou deux IV aléatoires passent à 31.",
      toxic: "Toxic Ball : empoisonne le Pokémon au lancer, le bonus de statut ×1,5 s'applique.",
      spark: "Spark Ball : paralyse le Pokémon au lancer, le bonus de statut ×1,5 s'applique.",
      scorch: "Scorch Ball : brûle le Pokémon au lancer, le bonus de statut ×1,5 s'applique.",
      frost: "Frost Ball : gèle le Pokémon au lancer, le bonus de statut ×2,5 s'applique.",
      pure: "Pure Ball : censée donner ×3,5 sans problème de statut. Le code du jeu ne l'applique jamais, donc ×1 ici.",
      status: "Status Ball : censée donner ×2,5 avec un problème de statut. Le code du jeu l'applique toujours, donc toujours ×2,5 ici.",
      candy: "Candy Ball : ×0,8, le Pokémon a 5 niveaux de plus après la capture.",
      rocket: "Rocket Ball : aucun bonus de capture.",
      fusion: "Fusion Ball : ×3 contre les fusions (appliqué automatiquement dès qu'un corps est choisi).",
    },
    lastBallLabel: "Dernière Ball de ce type dans le sac",
    lastBallHint: "Une capture critique devient possible : un seul contrôle de secousse au lieu de quatre.",
    dexOwnedLabel: "Espèces capturées dans le Pokédex",
    dexOwnedOptions: ["jusqu'à 300", "301–450", "451–600", "plus de 600"],
    caughtNoFusion:
      "Les fusions ne peuvent pas être enregistrées ici – enregistre la tête comme rencontre, puis fusionne-la depuis l'équipe.",
  },
  typen: {
    heading: "Table des types",
    battleHeading: "Combat",
    analyzeHeading: "Combat & Capture",
    viewWild: "Sauvage",
    viewTrainer: "Dresseur",
    battleHint: "Choisis un adversaire pour voir ses types, ses attaques et la comparaison d'équipe.",
    levelLabel: "Niveau",
    moveTypes: "Types d'attaques",
    damagingOnly: "attaques offensives uniquement",
    bodyPickerLabel: "Corps (optionnel, pour les fusions)",
    noMoveData: "Aucune donnée d'attaque n'est disponible pour ce jeu.",
    teamMatchup: "Ton équipe résiste-t-elle ?",
    explosionWarn: (name: string, lvl: number) => `⚠ Peut utiliser ${name} dès le N. ${lvl} !`,
    hint: "Choisis un Pokémon pour voir ses faiblesses et résistances.",
    weak4: "4× Faible contre",
    weak2: "2× Faible contre",
    resist2: "0,5× Résiste à",
    resist4: "0,25× Résiste fortement à",
    immune: "0× Immunisé contre",
    attack: "Attaque",
    defense: "Défense",
    statsHeading: "Stats",
    defenderWeakHeading: "Faiblesses en défense",
    teamStrengthHeading: "Forces contre mon équipe",
    matrixHeading: "Matrice des types",
    legendOffense: "Vert = vous frappez fort · Rouge = vous frappez faiblement · Noir = sans effet",
    legendDefense: "Vert = votre équipe encaisse peu · Rouge = votre équipe encaisse beaucoup · Noir = immunisé",
    addCard: "Ajouter un adversaire",
  },
  tms: {
    heading: "Compatibilité CT",
    searchPlaceholder: "Rechercher une capacité …",
    noResults: "Aucune capacité trouvée",
    hint: "Choisis une capacité pour voir quels Pokémon peuvent l'apprendre officiellement.",
    caveat: "Source : données officielles du jeu (compatibilité d'origine) – pas l'attribution aléatoire de la ROM.",
    notLearnable: "Cette capacité n'est ni une CT/CS ni une capacité de tuteur dans le jeu d'origine.",
    empty: "Aucun Pokémon vivant capturé pour l'instant.",
    teamHeading: "Équipe",
    bankHeading: "Banc",
    method: { tm: "CT", hm: "CS", tutor: "Tuteur", level: "Niv." },
    noMoveData: "Aucune donnée de CT ni d'attaque n'est disponible pour ce jeu.",
  },
  overview: {
    heading: "Aperçu",
    progress: "Progression",
    encounterProgress: "Rencontres",
    battleProgress: "Aventure",
    badgesHeading: "Badges",
    badgesEmpty: "Aucun badge n'est configuré pour ce jeu.",
    dashboard: "Tableau de bord",
    coverage: "Couverture d'équipe",
    offensiveGaps: "Lacunes offensives",
    offensiveGapsInfo: (level: number) =>
      `Seuls les types d'attaque que l'équipe a réellement appris jusqu'au dernier plafond de niveau franchi (niveau ${level}) sont pris en compte.`,
    noGaps: "Aucune lacune – l'équipe touche chaque type super-efficacement.",
    offensiveHint: "Types qu'aucun membre ne touche super-efficacement.",
    memorial: "Cimetière",
    memorialEmpty: "Aucune perte pour l'instant. Reste prudent.",
    deathPoint: "Moment du décès",
    deathPointNone: "Aucun moment",
    deathPointUnset: "Indiquer le moment …",
    totalDeaths: "Morts au total",
    teamBst: "BST d'équipe (actuel → max)",
    levelCap: "Plafond de niveau (actuel → suivant)",
    deathTallyHeading: "Responsable des morts",
    deathTallyUnattributed: (n: number) => `+${n} sans attribution`,
    player: "Joueur",
    team: "Équipe",
    caught: "Capturés",
    bankBstCol: "BST du banc",
    bankBstColHint: "BST des Pokémon capturés et vivants hors de l'équipe (actuel → pleinement évolué)",
    teamBstCol: "BST équipe",
    teamBstColHint: "BST des Pokémon de l'équipe (actuel → pleinement évolué)",
    emptyRun: "Aucun Pokémon capturé pour l'instant.",
    emptyRunHint: "Dès que tu captures quelque chose dans l'onglet Rencontres, le BST, la couverture et les stats apparaîtront ici.",
    bankBst: "BST en boîte (actuel → max)",
    caughtTotal: "Capturés",
  },
  actions: {
    unknownPokemon: (id: number) => `ID de Pokémon inconnu : ${id}`,
    unknownRoute: (id: number) => `ID de route inconnu : ${id}`,
    speciesLocked: (pokemonName: string, playerLabel: string, routeName: string) =>
      `${pokemonName} est bloqué par la Species Clause (déjà utilisé par ${playerLabel} sur ${routeName}).`,
    playerNotInRun: "Ce run n'a pas ce joueur.",
    encounterNotFound: (id: number) => `Rencontre #${id} introuvable.`,
    soulLinkNotFound: (id: number) => `SoulLink #${id} introuvable.`,
    runNotFound: "Run introuvable - le lien est invalide ou le run a été supprimé.",
    notOnPublicInstance: "Ce n'est pas disponible sur le site public.",
    deadCannotEvolve: "Un Pokémon mort ne peut pas évoluer.",
    deadCannotRevert: "Impossible d'annuler l'évolution d'un Pokémon mort.",
    invalidEvolutionTarget: "Cible d'évolution invalide.",
    invalidFormTarget: "Forme invalide pour ce Pokémon.",
    evolutionFamilyMismatch: "Erreur interne : incohérence de famille dans evolutions.json.",
    noPreEvolution: "Aucune pré-évolution.",
    unknownLevelCap: (id: number) => `Level cap inconnu : ${id}`,
    presetNotFound: (id: number) => `Règlement inconnu : ${id}`,
    nameRequired: "Le nom ne doit pas être vide.",
    backupInvalid: "Fichier de sauvegarde invalide ou corrompu.",
    backupEmpty: "La sauvegarde ne contient aucun run.",
    invalidTeamSlot: "Place d'équipe invalide.",
    fusionSameEncounter: "La tête et le corps doivent être deux captures différentes.",
    fusionDifferentPlayer: "La tête et le corps doivent appartenir au même joueur.",
    fusionNotCaught: "Les deux captures doivent être vivantes et attrapées pour fusionner.",
    fusionChain: "L'un des deux fait déjà partie d'une autre fusion.",
    fusionNotFound: "Cette carte n'est pas une fusion.",
    fusionDead: "Une fusion morte ne peut pas être modifiée.",
    fusionSplitDisabled:
      "La règle « les fusions sauvages peuvent être séparées » est désactivée dans ce run.",
    unexpected:
      "Erreur inattendue - l'action n'a pas été appliquée. Voir le journal du serveur.",
  },
};

const es: Dictionary = {
  header: {
    titleShort: "SoulLink Tracker",
    titleFull: "SoulLink Nuzlocke Tracker",
  },
  nav: {
    tracker: "Encuentros",
    links: "Equipo",
    pokedex: "Pokédex",
    typen: "Combate y captura",
    weaknesses: "Debilidades del equipo",
    overview: "Resumen",
    tms: "MT",
    catchrate: "Ratio de captura",
    levelcaps: "Aventura",
    rules: "Reglas",
  },
  theme: {
    toggleNeutral: "Cambiar tema",
    toDark: "Cambiar al tema oscuro",
    toLight: "Cambiar al tema claro",
  },
  language: {
    switchLabel: "Cambiar idioma",
  },
  dialog: {
    ok: "OK",
    cancel: "Cancelar",
    confirm: "Confirmar",
    saving: "Guardando…",
    saved: "Guardado",
  },
  menu: {
    copyRunLink: "Copiar enlace del run",
    runLinkCopied: "Enlace copiado. Quien lo tenga puede ver y editar el run.",
    runLinkManual: (url: string) => `Enlace del run: quien lo tenga puede ver y editar el run:\n\n${url}`,
    label: "Menú",
    language: "Idioma",
    version: "Versión",
    credits: "Créditos",
    creditsSprites: "Sprites de fusión",
    creditsSpritesSource: "Comunidad de sprites de Infinite Fusion (enlace directo)",
    creditsData: "Rutas y ubicaciones",
    creditsMechanics: "Mecánica de las Balls",
  },
  install: {
    menuLabel: "Instalar aplicación",
    iosTitle: "Instalar aplicación",
    iosHint:
      "En Safari, pulsa «Compartir» abajo y luego «Añadir a pantalla de inicio». " +
      "Nota: en iOS la aplicación instalada tiene su propio almacenamiento; el tema, el orden " +
      "de las pestañas y el idioma empiezan una vez con los valores predeterminados.",
    manualHint:
      "Tu navegador no ofrece instalación directa. Abre el menú del navegador y elige " +
      "«Añadir a pantalla de inicio» o «Instalar».",
  },
  blindflug: {
    name: "Vuelo a ciegas",
    hidden: "Oculto en vuelo a ciegas",
    enable: "Activar vuelo a ciegas",
    disable: "Desactivar vuelo a ciegas",
  },
  appearance: {
    label: "Apariencia",
    title: "Apariencia",
    sizeLabel: "Tamaño de la interfaz",
    smaller: "Pequeño",
    normal: "Normal",
    larger: "Grande",
    preview: "Así se ve el texto corrido en este tamaño.",
    previewSmall: "Y así las líneas pequeñas de debajo.",
    hint: "Solo vale para este dispositivo.",
    reset: "Restablecer",
    done: "Listo",
  },
  tabOrder: {
    label: "Ordenar pestañas",
    title: "Ordenar pestañas",
    moveUp: "Subir",
    moveDown: "Bajar",
    reset: "Restablecer",
    done: "Listo",
    hide: "Ocultar pestaña",
    show: "Mostrar pestaña",
    visibilityHint: "Desmarca para ocultar una pestaña.",
  },
  backup: {
    menuLabel: "Copia de seguridad",
    backupRun: "Guardar el run actual",
    backupAll: "Guardar todos los runs",
    import: "Importar copia …",
    importSuccess: (n: number) => `${n} run(s) importado(s).`,
    importNewerVersion:
      "Esta copia de seguridad se creó con una versión más reciente de la app. Algunos datos (p. ej. las fusiones) podrían no importarse.",
    noRun: "Ningún run seleccionado.",
    importTitle: "Importar copia de seguridad",
    importIntro: "Da un nombre a cada run de la copia antes de añadirlo como run nuevo.",
    importNameLabel: (i: number, total: number) =>
      total > 1 ? `Nombre para el run ${i + 1} de ${total}` : "Nombre para el nuevo run",
    importOriginalName: (name: string) => `En la copia: "${name}"`,
    importUseBackupName: "Usar nombre de la copia",
    importNameCollision: "Ya existe un run con este nombre.",
    importConfirm: "Importar",
    cancel: "Cancelar",
  },
  landing: {
    title: "Nuzlocke Tracker",
    empty: "Todavía no hay ningún run abierto en este dispositivo.",
    hint: "Crea un run nuevo. Para unirte a un SoulLink, abre el enlace que te han enviado tus compañeros.",
    create: "Crear un run nuevo",
    linkNote: "Cada run tiene su propio enlace secreto. Quien lo conozca puede ver y editar el run: compártelo solo con tus compañeros.",
  },
  runSwitcher: {
    newRun: "+ Nuevo run",
    rename: "Renombrar",
    renameTitle: "Renombrar run",
    renameLabel: (name: string) => `Renombrar el run "${name}"`,
    save: "Guardar",
    deleteButton: "Eliminar run",
    deleteLabel: (name: string) => `Eliminar el run "${name}"`,
    newRunTitle: "Nuevo run",
    nameLabel: "Nombre",
    namePlaceholder: "p. ej. Playthrough 2",
    gameLabel: "Juego",
    modeLabel: "Modo",
    modeSoullink: "SoulLink",
    modeClassic: "Solo",
    cancel: "Cancelar",
    create: "Crear",
    confirmDelete1: (name: string) =>
      `¿Eliminar de verdad el run "${name}"? Todos los encuentros, enlaces y el progreso de level caps de este run se perderán para siempre.`,
    soloSuffix: " (Solo)",
    playersLabel: "Jugadores",
    playersSuffix: (n: number) => ` (${n} jugadores)`,
  },
  player: {
    PLAYER1: "Jugador 1",
    PLAYER2: "Jugador 2",
    PLAYER3: "Jugador 3",
    PLAYER4: "Jugador 4",
  },
  status: {
    CAUGHT: "Capturado",
    KILLED: "Muerto",
    FLED: "Huido",
  },
  tracker: {
    custom: {
      manage: "Rutas propias",
      title: "Rutas propias",
      nameLabel: "Nombre",
      namePlaceholder: "p. ej. Zona Safari (extra)",
      typeLabel: "Tipo",
      typeRoute: "Ruta",
      typeStatic: "Estático",
      positionLabel: "Insertar después de",
      positionTop: "(al principio)",
      add: "Añadir",
      existing: "Rutas propias de este run",
      none: "Aún no hay rutas propias.",
      hint: "Cuenta en la barra de progreso y solo vale para este run.",
      delete: "Eliminar",
      deleteConfirm: (n: number) =>
        n === 0
          ? "¿Eliminar de verdad?"
          : `¿Eliminar de verdad? Se borrarán ${n} encuentros con ella.`,
      deleteYes: "Sí, eliminar",
      added: (name: string) => `«${name}» añadida.`,
      deleted: (name: string) => `«${name}» eliminada.`,
    },
    debug: {
      export: "Exportar orden",
      title: "Orden de encuentros",
      hint: "Para corregir el routes.json del paquete: copia el texto y dáselo a Claude Code.",
      copy: "Copiar",
      copied: "Copiado.",
      copyFailed: "No se pudo copiar: selecciona el texto a mano.",
      download: "Guardar como archivo",
    },
    heading: "Encuentros",
    searchPlaceholder: "Buscar Pokémon...",
    noResults: "Sin resultados",
    locked: "bloqueado",
    clearSelection: "Borrar selección",
    shinyToggle: "Marcar como shiny (Cláusula Shiny)",
    bodyOfFusion: (head: string, route: string) => `Cuerpo de ${head} (${route})`,
    fusedWith: (body: string, route: string) => `Fusionado con ${body} (${route})`,
    caughtAsFusion: "Capturado como fusión",
    typeStatic: "Estático",
    openOnly: "Solo abiertas",
    openOnlyTitle: "Mostrar solo rutas sin entrada completa",
    missingPlayer: "Aún falta el encuentro de al menos un jugador",
    clear: "Vaciar",
    clearConfirm: "¿Eliminar este encuentro?",
    nicknamePlaceholder: "Mote",
    nicknameLabel: "Mote",
    postgameShow: (n: number) => `Mostrar contenido posjuego (${n} zonas)`,
    postgameHide: "Ocultar contenido posjuego",
    allDone: "Todas las rutas completadas",
    showAll: "Mostrar todo",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} de ${total} rutas completadas (${percent}%)`,
  },
  links: {    free: {
      add: "Añadir Pokémon",
      title: "Añadir al equipo",
      pokemonLabel: "Pokémon",
      playerLabel: "Para",
      nicknameLabel: "Apodo",
      submit: "Añadir",
      added: (name: string) => `${name} añadido.`,
      removed: (name: string) => `${name} eliminado.`,
      remove: "Quitar",
      removeConfirm: "¿Quitar de verdad?",
      removeYes: "Sí, quitar",
      hint: "Crea su propia entrada en segundo plano. No cuenta en el progreso de encuentros.",
    },

    emptySoullink:
      "Aún no hay SoulLinks – captura tu primer Pokémon en la pestaña Encuentros.",
    emptyClassic: "Aún no hay Pokémon – captura el primero en la pestaña Encuentros.",
    markDead: "Marcar como muerto",
    clearTeam: "Vaciar equipo",
    clearTeamConfirm: "¿Vaciar los seis huecos del equipo? Los Pokémon se conservan.",
    dead: "Muerto",
    deadBy: (name: string) => `perdido por ${name}`,
    whoLost: "¿Quién perdió?",
    noAttribution: "Sin asignar",
    deathCausePlaceholder: "Causa de la muerte (opcional)",
    revive: "Revivir",
    evolve: "Evolucionar",
    revert: "Involucionar",
    staticTag: "estático",
    rankSummary: (rang: number, summe: number) => `Rango #${rang} · BST ${summe}`,
    fusionSummary: (summe: number) => `Fusión · BST ${summe}`,
    fuse: "Fusionar",
    fuseDialogTitle: "Fusionar",
    fuseDialogHint:
      "Fusiona esta captura (cabeza) con otra captura tuya (cuerpo). Ambas siguen siendo capturas separadas - solo la tarjeta las muestra juntas a partir de ahora.",
    fuseDonorLabel: "Cuerpo",
    fuseNoCandidates: "No hay otros Pokémon capturados con los que fusionar.",
    fuseNoCustomSprite: "Esta combinación no tiene sprite dibujado a mano, solo uno generado automáticamente.",
    fuseCdnUnreachable: "Servidor de sprites no disponible.",
    fuseSubmit: "Fusionar",
    fuseSuccess: "Fusionado.",
    swap: "Intercambiar",
    swapTitle: "Intercambiar cabeza y cuerpo",
    unfuse: "Separar",
    unfuseConfirm: "¿Separar la fusión?",
    fusionTag: "Fusión",
    bodyLabel: "Cuerpo",
    slotTakenBy: (routes: string) => `Ocupado por ${routes}`,
    slotTakenHint:
      "Este grupo necesita otro puesto mientras un jugador tenga en él más Pokémon separados que el otro.",
    sortLabel: "Orden",
    sortDefault: "Orden de encuentro",
    sortSumme: "BST",
    sortSummeMax: "Max BST",
    filterEvolvable: "Evolucionable",
    filterEvolvableTitle:
      "Mostrar solo Pokémon con evolución por nivel disponible ahora",
    hideTeam: "Ocultar equipo",
    hideTeamTitle: "Ocultar los Pokémon del equipo",
    filterTypes: "Tipos",
    filterTypesTitle: "Filtrar por tipo",
    filterTypesClear: "Restablecer",
    noMatches: "Ningún Pokémon coincide con los filtros",
    teamHeading: "Equipo actual",
    teamSlotLabel: (n: number) => `Puesto ${n}`,
    teamEmpty: "Vacío",
    teamSelectPlaceholder: "— Vacío —",
    addToTeam: "Al equipo",
    replaceHint: "¿A quién reemplazar?",
  },
  pokedex: {
    heading: "Pokédex",
    searchPlaceholder: "Buscar Pokémon...",
    noResults: "Sin resultados",
    filterAll: "Todos",
    filterAvailable: "Solo disponibles",
    filterLocked: "Solo bloqueados",
    filterLegendary: "Solo legendarios",
      resetFilters: "Restablecer filtros",
    columns: {
      id: "ID",
      name: "Nombre",
      types: "Tipos",
      kp: "PS",
      ang: "At.",
      vert: "Def.",
      spA: "At. Esp.",
      spV: "Def. Esp.",
      spez: "Especial",
      init: "Vel.",
      rang: "Rango",
      summe: "BST",
    },
    detail: {
      rank: "Rango",
      weaknesses: "Debilidades de tipo",
      stats: "Estadísticas",
      moves: "Ataques (por nivel)",
      evolution: "Evolución",
      noEvolution: "Sin evolución",
      fusionHead: "Cabeza",
      fusionBody: "Cuerpo",
      noMoveData: "No hay datos de ataques para este juego.",
      evolvesFrom: "Evoluciona de",
      level: (n: number) => `Nv. ${n}`,
      movePower: "Potencia",
      moveAccuracy: "Precisión",
      movePp: "PP",
      moveCategory: "Categoría",
      moveEffect: "Efecto",
      moveChance: (n: number) => `${n} % de probabilidad`,
      moveToday: (value: string) => `hoy ${value}`,
      moveClass: { physical: "Físico", special: "Especial", status: "Estado" },
      moveDetails: "Mostrar/ocultar detalles",
      weight: "Peso",
      dexNo: "N.º",
      forms: "Formas",
      formBase: "Forma normal",
      openInAnalyze: "Abrir en Combate y captura",
    },
  },
  errorPage: {
    title: "Algo ha salido mal",
    hint: "No se ha podido cargar la página. Volver a intentarlo suele bastar.",
    retry: "Reintentar",
  },
  levelcaps: {
    heading: "Aventura",
    maxLevel: "Nivel máx.",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} de ${total} combates completados (${percent}%)`,
    eliteFourMarker: "Aquí empieza el Alto Mando",
    empty: "No hay combates definidos en este paquete de juego.",
  },
  weaknesses: {
    heading: "Debilidades del equipo",
    empty: "No hay equipo – elige hasta 6 Pokémon en la pestaña Pokémon.",
    attackType: "Tipo de ataque",
    weakHeader: "Débil",
    resistHeader: "Resiste",
    criticalHint:
      "⚠ = debilidad crítica: este tipo de ataque golpea de forma supereficaz al menos a la mitad del equipo sin que nadie lo resista.",
  },
  rules: {
    debugHeading: "Depuración",
    heading: "Reglas",
    edit: "Editar",
    save: "Guardar",
    cancel: "Cancelar",
    loadDefault: "Cargar predeterminado",
    loadDefaultConfirm: "¿Reemplazar las reglas actuales por el reglamento predeterminado? (Se aplica solo al pulsar Guardar.)",
    settingsHeading: "Ajustes de reglas",
    playerNamesHeading: "Nombres de jugadores",
    playerNamesHint: "Sustituye «Jugador 1», «Jugador 2» … en este run.",
    presets: {
      label: "Reglamento",
      placeholder: "Cargar reglamento…",
      manage: "Reglamentos",
      dialogTitle: "Reglamentos",
      nameLabel: "Nombre del reglamento",
      namePlaceholder: "p. ej. SoulLink estándar",
      saveCurrent: "Guardar",
      overwrite: "Sobrescribir",
      overwriteHint: (name: string) => `«${name}» ya existe y se reemplazará.`,
      existing: "Reglamentos guardados",
      none: "Aún no hay reglamentos guardados.",
      scopeHint: "Los reglamentos son globales y están disponibles en todos los runs.",
      editingHint: "Guarda primero las notas.",
      delete: "Eliminar",
      deleteConfirm: "¿Eliminar de verdad?",
      deleteYes: "Sí, eliminar",
      applyConfirm: (name: string) =>
        `¿Cargar «${name}»? Se sobrescribirán los ajustes de reglas y las reglas propias de este run.`,
      applied: (name: string) => `«${name}» cargado.`,
      saved: (name: string) => `«${name}» guardado.`,
      deleted: (name: string) => `«${name}» eliminado.`,
    },
    notesHeading: "Reglas propias y notas",
    showNotes: "Mostrar",
    hideNotes: "Ocultar",
    toggles: {
      freeTeam: {
        label: "Equipo libre (sin Nuzlocke)",
        description:
          "Para una partida normal: la pestaña Equipo recibe un botón «añadir Pokémon» en vez de tener que introducir cada miembro por una ruta. La pestaña Encuentros sigue funcionando.",
      },
      customSpritesOnly: {
        label: "Solo sprites dibujados a mano",
        description:
          "Al fusionar aparece un aviso si la combinación elegida no tiene un sprite dibujado a mano, solo uno generado automáticamente. Nunca bloquea la fusión.",
      },
      randomizerFuseEverything: {
        label: "Randomizer: fusionar todo",
        description:
          "Para partidas con la opción «Fuse everything» del randomizer: cada Pokémon salvaje ya es una fusión, así que la pestaña Encuentros muestra el campo del cuerpo directamente, sin casilla. Si se deja vacío, la captura no es una fusión.",
      },
      speciesClausePerPlayer: {
        label: "Species Clause por jugador",
        description:
          "Cada jugador tiene su propia Species Clause: un encuentro bloquea la familia solo para el jugador que lo tuvo, ya sea capturado, huido o derrotado.",
      },
      speciesClauseLinkedLocksAll: {
        label: "Los enlaces capturados bloquean para todos",
        description:
          "Solo con «por jugador»: en cuanto se forma un enlace porque todos los jugadores capturaron en la ruta, la familia queda agotada para todos. Los Pokémon que huyeron o fueron derrotados siguen bloqueando solo a su propio jugador.",
      },
      wildFusionSplit: {
        label: "Las fusiones salvajes se pueden separar",
        description:
          "Una captura que ya era una fusión puede separarse en dos Pokémon, igual que en el juego. Desactivado = esa captura permanece como una sola unidad. Las fusiones propias no se ven afectadas.",
      },
      fusionLocksBothFamilies: {
        label: "Las fusiones salvajes bloquean ambas familias",
        description:
          "Una fusión capturada en estado salvaje bloquea las familias evolutivas de ambas especies, como dos capturas distintas. Desactivado = solo bloquea la cabeza.",
      },
      blindflug: {
        label: "Vuelo a ciegas",
        description:
          "Oculta todo lo que la aplicación calcula por ti: la efectividad de tipos en todas partes y las listas y descripciones de movimientos. La tabla de tipos permanece. Vale para todo el run.",
      },
      debugMode: {
        label: "Depuración: registrar el orden de encuentros",
        description:
          "Muestra en la pestaña Encuentros una exportación del orden en que se introdujeron las rutas, para corregir el routes.json de un paquete. El orden siempre se registra; esto solo muestra la exportación, y solo en este dispositivo.",
      },
      speciesClause: {
        label: "Species Clause",
        description:
          "Avisa cuando una familia evolutiva ya se ha usado en este run. Nunca bloquea el guardado.",
      },
      shinyClause: {
        label: "Cláusula Shiny",
        description:
          "Una captura marcada como shiny queda exenta de la cláusula de especie (los shinies siempre pueden capturarse).",
      },
      staticsExemptFromClause: {
        label: "Estáticos exentos de la Species Clause",
        description:
          "En lugares estáticos (regalos, fósiles, legendarios) no se muestra el aviso.",
      },
      nicknames: {
        label: "Motes",
        description:
          "Campo de texto en la pestaña Encuentros; las tarjetas de la pestaña Pokémon muestran el mote.",
      },
      evolutionOverridesImpossible: {
        label: "Randomizer: evoluciones imposibles cambiadas",
        description:
          "Evoluciones por intercambio, etc., reemplazadas según evolution-overrides.json (p. ej. Kadabra → al nivel 37). Desactivado = métodos originales.",
      },
      evolutionOverridesEasier: {
        label: "Randomizer: evoluciones facilitadas",
        description:
          "Niveles de evolución reducidos según evolution-overrides.json (p. ej. Dragonair → al nivel 40). Desactivado = niveles originales.",
      },
      evolutionOverridesTimeBased: {
        label: "Randomizer: evoluciones por hora eliminadas",
        description:
          "Condiciones de día/noche eliminadas (Eevee → Espeon/Umbreon con Piedra Solar/Lunar). Desactivado = condiciones originales.",
      },
      statics: {
        label: "Encuentros estáticos",
        description:
          "Muestra los lugares estáticos en la pestaña Encuentros. Desactivado = solo se registran rutas normales.",
      },
    },
  },
  catchrate: {
    heading: "Ratio de captura",
    hint: "Elige un Pokémon para calcular la probabilidad de captura.",
    lockWarning: "Este Pokémon está bloqueado por la Species Clause en este run.",
    lockWarningPlayer: "Bloqueado por la Species Clause para este jugador.",
    caughtHeading: "¿Pokémon capturado?",
    caughtNeedSelection: "Elige primero un Pokémon arriba.",
    caughtNoRoutes: "No quedan rutas abiertas.",
    selectRoute: "Elegir ruta …",
    caughtDone: (name: string, route: string) => `${name} registrado como capturado en ${route}.`,
    confirmCatch: "Capturar",
    conditionMet: "Condición cumplida",
    addCard: "Añadir tarjeta",
    ballLabel: "Poké Ball",
    hpLabel: "PS (%)",
    levelLabel: "Nivel del Pokémon salvaje",
    ownLevelLabel: "Nivel de tu Pokémon",
    statusLabel: "Problema de estado",
    turnLabel: "Turno de combate",
    resultLabel: "Probabilidad por lanzamiento",
    guaranteed: "Captura garantizada",
    avgThrows: (n: string) => `Ø ${n} lanzamientos hasta capturar`,
    details: (rate: number, ball: string, status: string) =>
      `Ratio base ${rate} · Ball ${ball} · Estado ${status}`,
    balls: {
      poke: "Poké Ball",
      great: "Super Ball",
      ultra: "Ultra Ball",
      master: "Master Ball",
      safari: "Safari Ball",
      net: "Malla Ball",
      nest: "Nido Ball",
      dive: "Buceo Ball",
      repeat: "Acopio Ball",
      timer: "Turno Ball",
      luxury: "Lujo Ball",
      premier: "Honor Ball",
      level: "Nivel Ball",
      lure: "Cebo Ball",
      moon: "Luna Ball",
      friend: "Amigo Ball",
      love: "Amor Ball",
      fast: "Rapid Ball",
      park: "Parque Ball",
      quick: "Veloz Ball",
      dusk: "Ocaso Ball",
      heal: "Cura Ball",
      dream: "Ensueño Ball",
      heavy: "Peso Ball",
      sport: "Competi Ball",
      cherish: "Gloria Ball",
      gender: "Gender Ball",
      boost: "Boost Ball",
      ability: "Ability Ball",
      virus: "Virus Ball",
      glitter: "Glitter Ball",
      perfect: "Perfect Ball",
      toxic: "Toxic Ball",
      spark: "Spark Ball",
      scorch: "Scorch Ball",
      frost: "Frost Ball",
      pure: "Pure Ball",
      status: "Status Ball",
      candy: "Candy Ball",
      rocket: "Rocket Ball",
      fusion: "Fusion Ball",
    },
    statusOptions: {
      none: "Ninguno",
      sleep: "Dormido",
      freeze: "Congelado",
      paralysis: "Parálisis",
      poison: "Envenenado",
      burn: "Quemado",
    },
    ballNotes: {
      net: "Malla Ball: ×3 solo contra Pokémon de tipo Agua y Bicho (se aplica automáticamente según el tipo).",
      nest: "Nido Ball: el bono depende del nivel del Pokémon salvaje (hasta ×3,9 a nivel 1, sin bono desde el nivel 30).",
      dive: "Buceo Ball: ×3,5 solo al surfear/bucear/pescar – se asume aquí.",
      repeat: "Acopio Ball: ×3 solo si la especie ya está registrada en la Pokédex – se asume aquí.",
      timer: "Turno Ball: el bono crece con el turno (máx. ×4 desde el turno 30).",
      level: "Nivel Ball: ×2 si tu Pokémon tiene más nivel que el salvaje, ×4 desde el doble y ×8 desde el cuádruple.",
      lure: "Cebo Ball: ×3 solo al pescar – se asume aquí.",
      moon: "Luna Ball: ×4 solo para familias de Piedra Lunar (p. ej. Nidoran, Clefairy) – se asume aquí.",
      friend: "Amigo Ball: sin bono de captura, sube la amistad tras la captura.",
      love: "Amor Ball: ×8 solo con la misma especie del sexo contrario – se asume aquí.",
      fast: "Rapid Ball: ×4 solo con especies que huyen – se asume aquí.",
      park: "Parque Ball: ×1,5 durante el Concurso de Captura.",
      quick: "Veloz Ball: ×4 solo en el turno 1, después ×1.",
      dusk: "Ocaso Ball: ×3,5 solo de noche o en cuevas – se asume aquí.",
      heal: "Cura Ball: sin bonus de captura, cura por completo al Pokémon capturado.",
      dream: "Ensueño Ball: captura garantizada en la 5.ª gen (solo Mundo Ensueño).",
      heavy: "Peso Ball: desplaza la tasa de captura según el peso (−20 por debajo de 102 kg, hasta +40 desde 410 kg) en vez de multiplicarla.",
      sport: "Competi Ball: sin bonus de captura, solo durante el Concurso de Captura.",
    },
    ballNotesInfiniteFusion: {
      nest: "Nido Ball: bono solo hasta el nivel 30, (41 − nivel) ÷ 10 – así que ×4 a nivel 1.",
      timer: "Turno Ball: ×1 en el turno 1, luego +0,3 por turno, hasta ×4.",
      quick: "Veloz Ball: ×5 solo en el turno 1, después ×1.",
      fast: "Rapid Ball: ×4 contra Pokémon con Velocidad base de 100 o más (se aplica automáticamente).",
      heavy: "Peso Ball: −20 por debajo de 204,8 kg, +20 desde 204,8 kg, +30 desde 307,2 kg, +40 desde 409,6 kg. Una fusión pesa la media de ambos Pokémon.",
      sport: "Competi Ball: ×1,5.",
      dream: "Ensueño Ball: duerme al Pokémon al lanzarla, así que se aplica el bono de sueño ×2,5. No es una captura garantizada.",
      cherish: "Gloria Ball: sin bonus de captura.",
      gender: "Gender Ball: sin bonus de captura, cambia el sexo del Pokémon tras capturarlo.",
      boost: "Boost Ball: ×0,8, el Pokémon cuenta como intercambiado tras capturarlo.",
      ability: "Ability Ball: ×0,6, el Pokémon obtiene su habilidad oculta.",
      virus: "Virus Ball: ×0,4, el Pokémon contrae Pokérus.",
      glitter: "Glitter Ball: ×0,2, el Pokémon se vuelve variocolor.",
      perfect: "Perfect Ball: ×0,1, uno o dos IV aleatorios pasan a 31.",
      toxic: "Toxic Ball: envenena al Pokémon al lanzarla, se aplica el bono de estado ×1,5.",
      spark: "Spark Ball: paraliza al Pokémon al lanzarla, se aplica el bono de estado ×1,5.",
      scorch: "Scorch Ball: quema al Pokémon al lanzarla, se aplica el bono de estado ×1,5.",
      frost: "Frost Ball: congela al Pokémon al lanzarla, se aplica el bono de estado ×2,5.",
      pure: "Pure Ball: debería dar ×3,5 sin problema de estado. El código del juego nunca lo aplica, así que aquí ×1.",
      status: "Status Ball: debería dar ×2,5 con un problema de estado. El código del juego siempre lo aplica, así que aquí siempre ×2,5.",
      candy: "Candy Ball: ×0,8, el Pokémon tiene 5 niveles más tras capturarlo.",
      rocket: "Rocket Ball: sin bonus de captura.",
      fusion: "Fusion Ball: ×3 contra fusiones (se aplica automáticamente al elegir un cuerpo).",
    },
    lastBallLabel: "Última Ball de su tipo en la mochila",
    lastBallHint: "Entonces es posible una captura crítica: una sola comprobación de sacudida en lugar de cuatro.",
    dexOwnedLabel: "Especies capturadas en la Pokédex",
    dexOwnedOptions: ["hasta 300", "301–450", "451–600", "más de 600"],
    caughtNoFusion:
      "Aquí no se pueden registrar fusiones: registra la cabeza como encuentro y fusiónala desde el equipo.",
  },
  typen: {
    heading: "Efectividad de tipos",
    battleHeading: "Combate",
    analyzeHeading: "Combate y captura",
    viewWild: "Salvaje",
    viewTrainer: "Entrenador",
    battleHint: "Elige un rival para ver sus tipos, ataques y la comparación de equipo.",
    levelLabel: "Nivel",
    moveTypes: "Tipos de ataque",
    damagingOnly: "solo ataques de daño",
    bodyPickerLabel: "Cuerpo (opcional, para fusiones)",
    noMoveData: "No hay datos de ataques para este juego.",
    teamMatchup: "¿Aguanta tu equipo?",
    explosionWarn: (name: string, lvl: number) => `⚠ Puede usar ${name} desde Nv. ${lvl}!`,
    hint: "Elige un Pokémon para ver sus debilidades y resistencias.",
    weak4: "4× Débil contra",
    weak2: "2× Débil contra",
    resist2: "0,5× Resiste",
    resist4: "0,25× Resiste mucho",
    immune: "0× Inmune a",
    attack: "Ataque",
    defense: "Defensa",
    statsHeading: "Stats",
    defenderWeakHeading: "Debilidades como defensor",
    teamStrengthHeading: "Fortalezas contra mi equipo",
    matrixHeading: "Matriz de tipos",
    legendOffense: "Verde = golpeas fuerte · Rojo = golpeas flojo · Negro = sin efecto",
    legendDefense: "Verde = tu equipo recibe poco · Rojo = tu equipo recibe mucho · Negro = inmune",
    addCard: "Añadir rival",
  },
  tms: {
    heading: "Compatibilidad MT",
    searchPlaceholder: "Buscar movimiento …",
    noResults: "No se encontró ningún movimiento",
    hint: "Elige un movimiento para ver qué Pokémon pueden aprenderlo oficialmente.",
    caveat: "Fuente: datos oficiales del juego (compatibilidad original), no la asignación aleatoria de la ROM.",
    notLearnable: "Este movimiento no es una MT/MO ni un movimiento de tutor en el juego original.",
    empty: "Aún no has capturado ningún Pokémon vivo.",
    teamHeading: "Equipo",
    bankHeading: "Banca",
    method: { tm: "MT", hm: "MO", tutor: "Tutor", level: "Niv." },
    noMoveData: "No hay datos de MT ni de ataques para este juego.",
  },
  overview: {
    heading: "Resumen",
    progress: "Progreso",
    encounterProgress: "Encuentros",
    battleProgress: "Aventura",
    badgesHeading: "Medallas",
    badgesEmpty: "No hay medallas configuradas para este juego.",
    dashboard: "Panel",
    coverage: "Cobertura del equipo",
    offensiveGaps: "Huecos ofensivos",
    offensiveGapsInfo: (level: number) =>
      `Solo cuentan los tipos de ataque que el equipo ha aprendido realmente hasta el último tope de nivel superado (nivel ${level}).`,
    noGaps: "Sin huecos: el equipo golpea a cada tipo de forma súper eficaz.",
    offensiveHint: "Tipos que ningún miembro golpea de forma súper eficaz.",
    memorial: "Cementerio",
    memorialEmpty: "Aún sin bajas. Ten cuidado.",
    deathPoint: "Momento de la muerte",
    deathPointNone: "Sin momento",
    deathPointUnset: "Indicar el momento …",
    totalDeaths: "Muertes totales",
    teamBst: "BST del equipo (actual → máx)",
    levelCap: "Tope de nivel (actual → siguiente)",
    deathTallyHeading: "Responsable de muertes",
    deathTallyUnattributed: (n: number) => `+${n} sin asignar`,
    player: "Jugador",
    team: "Equipo",
    caught: "Capturados",
    bankBstCol: "BST banca",
    bankBstColHint: "BST de los Pokémon capturados y vivos fuera del equipo (actual → totalmente evolucionado)",
    teamBstCol: "BST equipo",
    teamBstColHint: "BST de los Pokémon del equipo (actual → totalmente evolucionado)",
    emptyRun: "Aún no has capturado ningún Pokémon.",
    emptyRunHint: "En cuanto captures algo en la pestaña Encuentros, aquí aparecerán BST, cobertura y estadísticas.",
    bankBst: "BST en caja (actual → max)",
    caughtTotal: "Capturados",
  },
  actions: {
    unknownPokemon: (id: number) => `ID de Pokémon desconocido: ${id}`,
    unknownRoute: (id: number) => `ID de ruta desconocido: ${id}`,
    speciesLocked: (pokemonName: string, playerLabel: string, routeName: string) =>
      `${pokemonName} está bloqueado por la Species Clause (ya usado por ${playerLabel} en ${routeName}).`,
    playerNotInRun: "Este run no tiene ese jugador.",
    encounterNotFound: (id: number) => `Encuentro #${id} no encontrado.`,
    soulLinkNotFound: (id: number) => `SoulLink #${id} no encontrado.`,
    runNotFound: "Run no encontrado: el enlace no es válido o el run se ha eliminado.",
    notOnPublicInstance: "Esto no está disponible en el sitio público.",
    deadCannotEvolve: "Un Pokémon muerto no puede evolucionar.",
    deadCannotRevert: "Un Pokémon muerto no puede involucionar.",
    invalidEvolutionTarget: "Objetivo de evolución no válido.",
    invalidFormTarget: "Forma no válida para este Pokémon.",
    evolutionFamilyMismatch: "Error interno: inconsistencia de familia en evolutions.json.",
    noPreEvolution: "No hay preevolución.",
    unknownLevelCap: (id: number) => `Level cap desconocido: ${id}`,
    presetNotFound: (id: number) => `Reglamento desconocido: ${id}`,
    nameRequired: "El nombre no puede estar vacío.",
    backupInvalid: "Archivo de copia inválido o dañado.",
    backupEmpty: "La copia no contiene runs.",
    invalidTeamSlot: "Puesto de equipo no válido.",
    fusionSameEncounter: "La cabeza y el cuerpo deben ser dos capturas distintas.",
    fusionDifferentPlayer: "La cabeza y el cuerpo deben pertenecer al mismo jugador.",
    fusionNotCaught: "Ambas capturas deben estar vivas y atrapadas para fusionarlas.",
    fusionChain: "Uno de los dos ya forma parte de otra fusión.",
    fusionNotFound: "Esa carta no es una fusión.",
    fusionDead: "Una fusión muerta no se puede modificar.",
    fusionSplitDisabled:
      "En esta partida la regla «las fusiones salvajes se pueden separar» está desactivada.",
    unexpected:
      "Error inesperado - la acción no se aplicó. Consulta el registro del servidor.",
  },
};

const it: Dictionary = {
  header: {
    titleShort: "SoulLink Tracker",
    titleFull: "SoulLink Nuzlocke Tracker",
  },
  nav: {
    tracker: "Incontri",
    links: "Squadra",
    pokedex: "Pokédex",
    typen: "Lotta e cattura",
    weaknesses: "Debolezze della squadra",
    overview: "Panoramica",
    tms: "MT",
    catchrate: "Tasso di cattura",
    levelcaps: "Viaggio",
    rules: "Regole",
  },
  theme: {
    toggleNeutral: "Cambia tema",
    toDark: "Passa al tema scuro",
    toLight: "Passa al tema chiaro",
  },
  language: {
    switchLabel: "Cambia lingua",
  },
  dialog: {
    ok: "OK",
    cancel: "Annulla",
    confirm: "Conferma",
    saving: "Salvataggio…",
    saved: "Salvato",
  },
  menu: {
    copyRunLink: "Copia il link del run",
    runLinkCopied: "Link copiato. Chi lo possiede può vedere e modificare il run.",
    runLinkManual: (url: string) => `Link del run: chi lo possiede può vedere e modificare il run:\n\n${url}`,
    label: "Menu",
    language: "Lingua",
    version: "Versione",
    credits: "Crediti",
    creditsSprites: "Sprite delle fusioni",
    creditsSpritesSource: "Community di sprite di Infinite Fusion (link diretto)",
    creditsData: "Percorsi e luoghi",
    creditsMechanics: "Meccanica delle Ball",
  },
  install: {
    menuLabel: "Installa l'app",
    iosTitle: "Installa l'app",
    iosHint:
      "In Safari tocca «Condividi» in basso e poi «Aggiungi a Home». " +
      "Nota: su iOS l'app installata ha un archivio proprio: tema, ordine delle schede e " +
      "lingua partono una volta dai valori predefiniti.",
    manualHint:
      "Il tuo browser non offre un'installazione diretta. Apri il menu del browser e scegli " +
      "«Aggiungi a Home» o «Installa».",
  },
  blindflug: {
    name: "Volo cieco",
    hidden: "Nascosto in volo cieco",
    enable: "Attiva il volo cieco",
    disable: "Disattiva il volo cieco",
  },
  appearance: {
    label: "Aspetto",
    title: "Aspetto",
    sizeLabel: "Dimensione dell’interfaccia",
    smaller: "Piccolo",
    normal: "Normale",
    larger: "Grande",
    preview: "Ecco come appare il testo a questa dimensione.",
    previewSmall: "E così le piccole note qui sotto.",
    hint: "Vale solo su questo dispositivo.",
    reset: "Ripristina",
    done: "Fatto",
  },
  tabOrder: {
    label: "Disponi schede",
    title: "Disponi schede",
    moveUp: "Su",
    moveDown: "Giù",
    reset: "Ripristina",
    done: "Fatto",
    hide: "Nascondi scheda",
    show: "Mostra scheda",
    visibilityHint: "Deseleziona per nascondere una scheda.",
  },
  backup: {
    menuLabel: "Backup e importazione",
    backupRun: "Salva il run attuale",
    backupAll: "Salva tutti i run",
    import: "Importa backup …",
    importSuccess: (n: number) => `${n} run importati.`,
    importNewerVersion:
      "Questo backup è stato creato con una versione più recente dell'app. Alcuni dati (es. le fusioni) potrebbero non essere importati.",
    noRun: "Nessun run selezionato.",
    importTitle: "Importa backup",
    importIntro: "Assegna un nome a ogni run del backup prima che venga aggiunto come nuovo run.",
    importNameLabel: (i: number, total: number) =>
      total > 1 ? `Nome per il run ${i + 1} di ${total}` : "Nome per il nuovo run",
    importOriginalName: (name: string) => `Nel backup: "${name}"`,
    importUseBackupName: "Usa il nome dal backup",
    importNameCollision: "Esiste già un run con questo nome.",
    importConfirm: "Importa",
    cancel: "Annulla",
  },
  landing: {
    title: "Nuzlocke Tracker",
    empty: "Su questo dispositivo non è ancora aperto nessun run.",
    hint: "Crea un nuovo run. Per unirti a un SoulLink, apri il link che ti hanno inviato i tuoi compagni.",
    create: "Crea un nuovo run",
    linkNote: "Ogni run ha un proprio link segreto. Chi lo conosce può vedere e modificare il run: condividilo solo con i tuoi compagni.",
  },
  runSwitcher: {
    newRun: "+ Nuovo run",
    rename: "Rinomina",
    renameTitle: "Rinomina run",
    renameLabel: (name: string) => `Rinomina il run "${name}"`,
    save: "Salva",
    deleteButton: "Elimina run",
    deleteLabel: (name: string) => `Elimina il run "${name}"`,
    newRunTitle: "Nuovo run",
    nameLabel: "Nome",
    namePlaceholder: "es. Playthrough 2",
    gameLabel: "Gioco",
    modeLabel: "Modalità",
    modeSoullink: "SoulLink",
    modeClassic: "Solo",
    cancel: "Annulla",
    create: "Crea",
    confirmDelete1: (name: string) =>
      `Eliminare davvero il run "${name}"? Tutti gli incontri, i link e i progressi dei level cap di questo run andranno persi per sempre.`,
    soloSuffix: " (Solo)",
    playersLabel: "Giocatori",
    playersSuffix: (n: number) => ` (${n} giocatori)`,
  },
  player: {
    PLAYER1: "Giocatore 1",
    PLAYER2: "Giocatore 2",
    PLAYER3: "Giocatore 3",
    PLAYER4: "Giocatore 4",
  },
  status: {
    CAUGHT: "Catturato",
    KILLED: "Ucciso",
    FLED: "Fuggito",
  },
  tracker: {
    custom: {
      manage: "Percorsi personali",
      title: "Percorsi personali",
      nameLabel: "Nome",
      namePlaceholder: "es. Zona Safari (extra)",
      typeLabel: "Tipo",
      typeRoute: "Percorso",
      typeStatic: "Statico",
      positionLabel: "Inserisci dopo",
      positionTop: "(all’inizio)",
      add: "Aggiungi",
      existing: "Percorsi personali di questo run",
      none: "Nessun percorso personale.",
      hint: "Conta nella barra di avanzamento e vale solo per questo run.",
      delete: "Elimina",
      deleteConfirm: (n: number) =>
        n === 0
          ? "Eliminare davvero?"
          : `Eliminare davvero? ${n} incontri verranno eliminati con esso.`,
      deleteYes: "Sì, elimina",
      added: (name: string) => `«${name}» aggiunto.`,
      deleted: (name: string) => `«${name}» eliminato.`,
    },
    debug: {
      export: "Esporta ordine",
      title: "Ordine degli incontri",
      hint: "Per correggere il routes.json del pacchetto: copia il testo e passalo a Claude Code.",
      copy: "Copia",
      copied: "Copiato.",
      copyFailed: "Copia non riuscita: seleziona il testo a mano.",
      download: "Salva come file",
    },
    heading: "Incontri",
    searchPlaceholder: "Cerca Pokémon...",
    noResults: "Nessun risultato",
    locked: "bloccato",
    clearSelection: "Cancella selezione",
    shinyToggle: "Contrassegna come shiny (Clausola Shiny)",
    bodyOfFusion: (head: string, route: string) => `Corpo di ${head} (${route})`,
    fusedWith: (body: string, route: string) => `Fuso con ${body} (${route})`,
    caughtAsFusion: "Catturato come fusione",
    typeStatic: "Statico",
    openOnly: "Solo aperti",
    openOnlyTitle: "Mostra solo i percorsi senza voce completa",
    missingPlayer: "Manca ancora l'incontro di almeno un giocatore",
    clear: "Svuota",
    clearConfirm: "Eliminare davvero questo incontro?",
    nicknamePlaceholder: "Soprannome",
    nicknameLabel: "Soprannome",
    postgameShow: (n: number) => `Mostra contenuti post-gioco (${n} aree)`,
    postgameHide: "Nascondi contenuti post-gioco",
    allDone: "Tutte le rotte sono completate",
    showAll: "Mostra tutto",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} di ${total} percorsi completati (${percent}%)`,
  },
  links: {    free: {
      add: "Aggiungi Pokémon",
      title: "Aggiungi alla squadra",
      pokemonLabel: "Pokémon",
      playerLabel: "Per",
      nicknameLabel: "Soprannome",
      submit: "Aggiungi",
      added: (name: string) => `${name} aggiunto.`,
      removed: (name: string) => `${name} rimosso.`,
      remove: "Rimuovi",
      removeConfirm: "Rimuovere davvero?",
      removeYes: "Sì, rimuovi",
      hint: "Crea una propria voce dietro le quinte. Non conta nel progresso degli incontri.",
    },

    emptySoullink:
      "Ancora nessun SoulLink – cattura il tuo primo Pokémon nella scheda Incontri.",
    emptyClassic: "Ancora nessun Pokémon – cattura il primo nella scheda Incontri.",
    markDead: "Segna come morto",
    clearTeam: "Svuota squadra",
    clearTeamConfirm: "Svuotare tutti e sei i posti? I Pokémon vengono mantenuti.",
    dead: "Morto",
    deadBy: (name: string) => `perso da ${name}`,
    whoLost: "Chi ha perso?",
    noAttribution: "Senza attribuzione",
    deathCausePlaceholder: "Causa della morte (facoltativo)",
    revive: "Rianima",
    evolve: "Evolvi",
    revert: "Annulla evoluzione",
    staticTag: "statico",
    rankSummary: (rang: number, summe: number) => `Rango #${rang} · BST ${summe}`,
    fusionSummary: (summe: number) => `Fusione · BST ${summe}`,
    fuse: "Fondi",
    fuseDialogTitle: "Fondi",
    fuseDialogHint:
      "Fonde questa cattura (testa) con un'altra tua cattura (corpo). Entrambe restano catture separate - solo la carta le mostra insieme d'ora in poi.",
    fuseDonorLabel: "Corpo",
    fuseNoCandidates: "Nessun altro Pokémon catturato con cui fondersi.",
    fuseNoCustomSprite: "Questa combinazione non ha uno sprite disegnato a mano, solo uno generato automaticamente.",
    fuseCdnUnreachable: "Server degli sprite non raggiungibile.",
    fuseSubmit: "Fondi",
    fuseSuccess: "Fuso.",
    swap: "Scambia",
    swapTitle: "Scambia testa e corpo",
    unfuse: "Separa",
    unfuseConfirm: "Separare davvero la fusione?",
    fusionTag: "Fusione",
    bodyLabel: "Corpo",
    slotTakenBy: (routes: string) => `Occupato da ${routes}`,
    slotTakenHint:
      "Questo gruppo richiede un altro posto finché un giocatore vi ha più Pokémon separati dell'altro.",
    sortLabel: "Ordinamento",
    sortDefault: "Ordine di incontro",
    sortSumme: "BST",
    sortSummeMax: "Max BST",
    filterEvolvable: "Evolvibile",
    filterEvolvableTitle:
      "Mostra solo i Pokémon con un'evoluzione di livello disponibile ora",
    hideTeam: "Nascondi squadra",
    hideTeamTitle: "Nascondi i Pokémon in squadra",
    filterTypes: "Tipi",
    filterTypesTitle: "Filtra per tipo",
    filterTypesClear: "Reimposta",
    noMatches: "Nessun Pokémon corrisponde ai filtri",
    teamHeading: "Squadra attuale",
    teamSlotLabel: (n: number) => `Posto ${n}`,
    teamEmpty: "Vuoto",
    teamSelectPlaceholder: "— Vuoto —",
    addToTeam: "In squadra",
    replaceHint: "Chi sostituire?",
  },
  pokedex: {
    heading: "Pokédex",
    searchPlaceholder: "Cerca Pokémon...",
    noResults: "Nessun risultato",
    filterAll: "Tutti",
    filterAvailable: "Solo disponibili",
    filterLocked: "Solo bloccati",
    filterLegendary: "Solo leggendari",
      resetFilters: "Reimposta i filtri",
    columns: {
      id: "ID",
      name: "Nome",
      types: "Tipi",
      kp: "PS",
      ang: "Att.",
      vert: "Dif.",
      spA: "Att. Sp.",
      spV: "Dif. Sp.",
      spez: "Speciale",
      init: "Vel.",
      rang: "Rango",
      summe: "BST",
    },
    detail: {
      rank: "Rango",
      weaknesses: "Debolezze di tipo",
      stats: "Statistiche base",
      moves: "Mosse (per livello)",
      evolution: "Evoluzione",
      noEvolution: "Nessuna evoluzione",
      fusionHead: "Testa",
      fusionBody: "Corpo",
      noMoveData: "Per questo gioco non ci sono dati sulle mosse.",
      evolvesFrom: "Evolve da",
      level: (n: number) => `Lv. ${n}`,
      movePower: "Potenza",
      moveAccuracy: "Precisione",
      movePp: "PP",
      moveCategory: "Categoria",
      moveEffect: "Effetto",
      moveChance: (n: number) => `${n}% di probabilità`,
      moveToday: (value: string) => `oggi ${value}`,
      moveClass: { physical: "Fisico", special: "Speciale", status: "Stato" },
      moveDetails: "Mostra/nascondi dettagli",
      weight: "Peso",
      dexNo: "N.",
      forms: "Forme",
      formBase: "Forma normale",
      openInAnalyze: "Apri in Lotta e cattura",
    },
  },
  errorPage: {
    title: "Qualcosa è andato storto",
    hint: "Impossibile caricare la pagina. Riprovare di solito basta.",
    retry: "Riprova",
  },
  levelcaps: {
    heading: "Viaggio",
    maxLevel: "Livello max",
    progressTitle: (done: number, total: number, percent: number) =>
      `${done} di ${total} combattimenti completati (${percent}%)`,
    eliteFourMarker: "Qui inizia la Superquattro",
    empty: "Nessuna lotta definita in questo pacchetto di gioco.",
  },
  weaknesses: {
    heading: "Debolezze della squadra",
    empty: "Nessuna squadra – scegli fino a 6 Pokémon nella scheda Pokémon.",
    attackType: "Tipo d'attacco",
    weakHeader: "Debole",
    resistHeader: "Resiste",
    criticalHint:
      "⚠ = debolezza critica: questo tipo d'attacco colpisce in modo superefficace almeno metà squadra senza che nessuno resista.",
  },
  rules: {
    debugHeading: "Debug",
    heading: "Regole",
    edit: "Modifica",
    save: "Salva",
    cancel: "Annulla",
    loadDefault: "Carica predefinito",
    loadDefaultConfirm: "Sostituire le regole attuali con il regolamento predefinito? (Applicato solo premendo Salva.)",
    settingsHeading: "Impostazioni delle regole",
    playerNamesHeading: "Nomi dei giocatori",
    playerNamesHint: "Sostituisce «Giocatore 1», «Giocatore 2» … in questo run.",
    presets: {
      label: "Regolamento",
      placeholder: "Carica regolamento…",
      manage: "Regolamenti",
      dialogTitle: "Regolamenti",
      nameLabel: "Nome del regolamento",
      namePlaceholder: "es. SoulLink standard",
      saveCurrent: "Salva",
      overwrite: "Sovrascrivi",
      overwriteHint: (name: string) => `«${name}» esiste già e verrà sostituito.`,
      existing: "Regolamenti salvati",
      none: "Nessun regolamento salvato.",
      scopeHint: "I regolamenti valgono per tutta l’app e sono disponibili in ogni run.",
      editingHint: "Salva prima le note.",
      delete: "Elimina",
      deleteConfirm: "Eliminare davvero?",
      deleteYes: "Sì, elimina",
      applyConfirm: (name: string) =>
        `Caricare «${name}»? Le impostazioni delle regole e le regole personali di questo run verranno sovrascritte.`,
      applied: (name: string) => `«${name}» caricato.`,
      saved: (name: string) => `«${name}» salvato.`,
      deleted: (name: string) => `«${name}» eliminato.`,
    },
    notesHeading: "Regole personali e note",
    showNotes: "Mostra",
    hideNotes: "Nascondi",
    toggles: {
      freeTeam: {
        label: "Squadra libera (senza Nuzlocke)",
        description:
          "Per una partita normale: la scheda Squadra riceve un pulsante «aggiungi Pokémon» invece di dover inserire ogni membro tramite un percorso. La scheda Incontri resta utilizzabile.",
      },
      customSpritesOnly: {
        label: "Solo sprite disegnati a mano",
        description:
          "La fusione mostra un avviso se la coppia scelta non ha uno sprite disegnato a mano, solo uno generato automaticamente. Non blocca mai la fusione.",
      },
      randomizerFuseEverything: {
        label: "Randomizer: fondi tutto",
        description:
          "Per le partite con l’opzione «Fuse everything» del randomizer: ogni Pokémon selvatico è già una fusione, quindi la scheda Incontri mostra subito il campo del corpo, senza casella. Se resta vuoto, la cattura non è una fusione.",
      },
      speciesClausePerPlayer: {
        label: "Species Clause per giocatore",
        description:
          "Ogni giocatore ha la propria Species Clause: un incontro blocca la famiglia solo per il giocatore che l'ha avuto, che sia catturato, fuggito o sconfitto.",
      },
      speciesClauseLinkedLocksAll: {
        label: "I link catturati bloccano per tutti",
        description:
          "Solo con «per giocatore»: quando si forma un link perché tutti i giocatori hanno catturato nel percorso, la famiglia è esaurita per tutti. I Pokémon fuggiti o sconfitti bloccano ancora solo il proprio giocatore.",
      },
      wildFusionSplit: {
        label: "Le fusioni selvatiche possono essere separate",
        description:
          "Una cattura che era già una fusione può essere separata in due Pokémon, proprio come nel gioco. Disattivato = quella cattura resta un'unica unità. Le fusioni fatte da te non sono interessate.",
      },
      fusionLocksBothFamilies: {
        label: "Le fusioni selvatiche bloccano entrambe le famiglie",
        description:
          "Una fusione catturata allo stato brado blocca le famiglie evolutive di entrambe le specie, come due catture separate. Disattivato = blocca solo la testa.",
      },
      blindflug: {
        label: "Volo cieco",
        description:
          "Nasconde tutto ciò che l’app calcola per te: l’efficacia dei tipi ovunque e gli elenchi e le descrizioni delle mosse. La tabella dei tipi resta. Vale per tutto il run.",
      },
      debugMode: {
        label: "Debug: registra l’ordine degli incontri",
        description:
          "Mostra nella scheda Incontri un’esportazione dell’ordine in cui i percorsi sono stati inseriti, per correggere il routes.json di un pacchetto. L’ordine viene sempre registrato; questo mostra solo l’esportazione, e solo su questo dispositivo.",
      },
      speciesClause: {
        label: "Species Clause",
        description:
          "Avvisa quando una famiglia evolutiva è già stata usata in questo run. Non blocca mai il salvataggio.",
      },
      shinyClause: {
        label: "Clausola Shiny",
        description:
          "Una cattura contrassegnata come shiny è esente dalla clausola specie (gli shiny possono sempre essere catturati).",
      },
      staticsExemptFromClause: {
        label: "Statici esenti dalla Species Clause",
        description:
          "Nei luoghi statici (regali, fossili, leggendari) non viene mostrato l'avviso.",
      },
      nicknames: {
        label: "Soprannomi",
        description:
          "Campo di testo nella scheda Incontri; le carte della scheda Pokémon mostrano il soprannome.",
      },
      evolutionOverridesImpossible: {
        label: "Randomizer: evoluzioni impossibili modificate",
        description:
          "Evoluzioni per scambio ecc. sostituite secondo evolution-overrides.json (es. Kadabra → al livello 37). Disattivato = metodi originali.",
      },
      evolutionOverridesEasier: {
        label: "Randomizer: evoluzioni facilitate",
        description:
          "Livelli di evoluzione ridotti secondo evolution-overrides.json (es. Dragonair → al livello 40). Disattivato = livelli originali.",
      },
      evolutionOverridesTimeBased: {
        label: "Randomizer: evoluzioni a tempo rimosse",
        description:
          "Condizioni giorno/notte rimosse (Eevee → Espeon/Umbreon con Pietrasolare/Pietralunare). Disattivato = condizioni originali.",
      },
      statics: {
        label: "Incontri statici",
        description:
          "Mostra i luoghi statici nella scheda Incontri. Disattivato = si tracciano solo i percorsi normali.",
      },
    },
  },
  catchrate: {
    heading: "Tasso di cattura",
    hint: "Scegli un Pokémon per calcolare la probabilità di cattura.",
    lockWarning: "Questo Pokémon è bloccato dalla Species Clause in questo run.",
    lockWarningPlayer: "Bloccato dalla Species Clause per questo giocatore.",
    caughtHeading: "Pokémon catturato?",
    caughtNeedSelection: "Scegli prima un Pokémon qui sopra.",
    caughtNoRoutes: "Nessun percorso aperto rimasto.",
    selectRoute: "Scegli un percorso …",
    caughtDone: (name: string, route: string) => `${name} registrato come catturato su ${route}.`,
    confirmCatch: "Cattura",
    conditionMet: "Condizione soddisfatta",
    addCard: "Aggiungi scheda",
    ballLabel: "Poké Ball",
    hpLabel: "PS (%)",
    levelLabel: "Livello del Pokémon selvatico",
    ownLevelLabel: "Livello del tuo Pokémon",
    statusLabel: "Problema di stato",
    turnLabel: "Turno di lotta",
    resultLabel: "Probabilità per lancio",
    guaranteed: "Cattura garantita",
    avgThrows: (n: string) => `Ø ${n} lanci fino alla cattura`,
    details: (rate: number, ball: string, status: string) =>
      `Tasso base ${rate} · Ball ${ball} · Stato ${status}`,
    balls: {
      poke: "Poké Ball",
      great: "Mega Ball",
      ultra: "Ultra Ball",
      master: "Master Ball",
      safari: "Safari Ball",
      net: "Rete Ball",
      nest: "Minor Ball",
      dive: "Sub Ball",
      repeat: "Bis Ball",
      timer: "Timer Ball",
      luxury: "Chic Ball",
      premier: "Honor Ball",
      level: "Livello Ball",
      lure: "Esca Ball",
      moon: "Luna Ball",
      friend: "Amico Ball",
      love: "Amore Ball",
      fast: "Rapid Ball",
      park: "Parco Ball",
      quick: "Velox Ball",
      dusk: "Scuro Ball",
      heal: "Curaball",
      dream: "Ball Sogno",
      heavy: "Peso Ball",
      sport: "Gara Ball",
      cherish: "Pregio Ball",
      gender: "Gender Ball",
      boost: "Boost Ball",
      ability: "Ability Ball",
      virus: "Virus Ball",
      glitter: "Glitter Ball",
      perfect: "Perfect Ball",
      toxic: "Toxic Ball",
      spark: "Spark Ball",
      scorch: "Scorch Ball",
      frost: "Frost Ball",
      pure: "Pure Ball",
      status: "Status Ball",
      candy: "Candy Ball",
      rocket: "Rocket Ball",
      fusion: "Fusion Ball",
    },
    statusOptions: {
      none: "Nessuno",
      sleep: "Sonno",
      freeze: "Congelato",
      paralysis: "Paralisi",
      poison: "Veleno",
      burn: "Scottatura",
    },
    ballNotes: {
      net: "Rete Ball: ×3 solo contro Pokémon di tipo Acqua e Coleottero (applicato automaticamente in base al tipo).",
      nest: "Minor Ball: il bonus dipende dal livello del Pokémon selvatico (fino a ×3,9 al livello 1, nessun bonus dal livello 30).",
      dive: "Sub Ball: ×3,5 solo mentre si surfa/ci si immerge/si pesca – qui dato per assunto.",
      repeat: "Bis Ball: ×3 solo se la specie è già registrata nel Pokédex – qui dato per assunto.",
      timer: "Timer Ball: il bonus cresce col turno (max ×4 dal turno 30).",
      level: "Livello Ball: ×2 se il tuo Pokémon ha un livello più alto di quello selvatico, ×4 dal doppio e ×8 dal quadruplo.",
      lure: "Esca Ball: ×3 solo pescando – qui dato per assunto.",
      moon: "Luna Ball: ×4 solo per famiglie da Pietralunare (es. Nidoran, Clefairy) – qui dato per assunto.",
      friend: "Amico Ball: nessun bonus di cattura, aumenta l'amicizia dopo la cattura.",
      love: "Amore Ball: ×8 solo con la stessa specie di sesso opposto – qui dato per assunto.",
      fast: "Rapid Ball: ×4 solo con specie che fuggono – qui dato per assunto.",
      park: "Parco Ball: ×1,5 durante la Gara di Cattura.",
      quick: "Velox Ball: ×4 solo al turno 1, poi ×1.",
      dusk: "Scuro Ball: ×3,5 solo di notte o nelle grotte – qui dato per assunto.",
      heal: "Curaball: nessun bonus di cattura, cura completamente il Pokémon catturato.",
      dream: "Ball Sogno: cattura garantita in 5ª gen (solo Mondo Sogno).",
      heavy: "Peso Ball: sposta il tasso di cattura in base al peso (−20 sotto 102 kg, fino a +40 da 410 kg) invece di moltiplicarlo.",
      sport: "Gara Ball: nessun bonus di cattura, solo durante la Gara Acchiappainsetti.",
    },
    ballNotesInfiniteFusion: {
      nest: "Minor Ball: bonus solo fino al livello 30, (41 − livello) ÷ 10 – quindi ×4 al livello 1.",
      timer: "Timer Ball: ×1 al turno 1, poi +0,3 per turno, fino a ×4.",
      quick: "Velox Ball: ×5 solo al turno 1, poi ×1.",
      fast: "Rapid Ball: ×4 contro Pokémon con Velocità base pari o superiore a 100 (applicato automaticamente).",
      heavy: "Peso Ball: −20 sotto 204,8 kg, +20 da 204,8 kg, +30 da 307,2 kg, +40 da 409,6 kg. Una fusione pesa la media dei due Pokémon.",
      sport: "Gara Ball: ×1,5.",
      dream: "Ball Sogno: addormenta il Pokémon al lancio, quindi vale il bonus Sonno ×2,5. Cattura non garantita.",
      cherish: "Pregio Ball: nessun bonus di cattura.",
      gender: "Gender Ball: nessun bonus di cattura, inverte il sesso del Pokémon dopo la cattura.",
      boost: "Boost Ball: ×0,8, il Pokémon conta come scambiato dopo la cattura.",
      ability: "Ability Ball: ×0,6, il Pokémon ottiene la sua abilità nascosta.",
      virus: "Virus Ball: ×0,4, il Pokémon prende il Pokérus.",
      glitter: "Glitter Ball: ×0,2, il Pokémon diventa cromatico.",
      perfect: "Perfect Ball: ×0,1, uno o due IV casuali diventano 31.",
      toxic: "Toxic Ball: avvelena il Pokémon al lancio, vale il bonus di stato ×1,5.",
      spark: "Spark Ball: paralizza il Pokémon al lancio, vale il bonus di stato ×1,5.",
      scorch: "Scorch Ball: scotta il Pokémon al lancio, vale il bonus di stato ×1,5.",
      frost: "Frost Ball: congela il Pokémon al lancio, vale il bonus di stato ×2,5.",
      pure: "Pure Ball: dovrebbe dare ×3,5 senza problemi di stato. Il codice del gioco non lo applica mai, quindi qui ×1.",
      status: "Status Ball: dovrebbe dare ×2,5 con un problema di stato. Il codice del gioco lo applica sempre, quindi qui sempre ×2,5.",
      candy: "Candy Ball: ×0,8, il Pokémon è di 5 livelli più alto dopo la cattura.",
      rocket: "Rocket Ball: nessun bonus di cattura.",
      fusion: "Fusion Ball: ×3 contro le fusioni (applicato automaticamente quando scegli un corpo).",
    },
    lastBallLabel: "Ultima Ball del suo tipo nella borsa",
    lastBallHint: "Allora è possibile una cattura critica: un solo controllo di scossa invece di quattro.",
    dexOwnedLabel: "Specie catturate nel Pokédex",
    dexOwnedOptions: ["fino a 300", "301–450", "451–600", "oltre 600"],
    caughtNoFusion:
      "Qui non si possono registrare fusioni: registra la testa come incontro e fondila dalla squadra.",
  },
  typen: {
    heading: "Efficacia dei tipi",
    battleHeading: "Lotta",
    analyzeHeading: "Lotta e cattura",
    viewWild: "Selvatico",
    viewTrainer: "Allenatore",
    battleHint: "Scegli un avversario per vederne tipi, attacchi e il confronto con la squadra.",
    levelLabel: "Livello",
    moveTypes: "Tipi di attacco",
    damagingOnly: "solo attacchi offensivi",
    bodyPickerLabel: "Corpo (facoltativo, per le fusioni)",
    noMoveData: "Per questo gioco non ci sono dati sulle mosse.",
    teamMatchup: "Quanto regge la tua squadra?",
    explosionWarn: (name: string, lvl: number) => `⚠ Può usare ${name} dal Lv ${lvl}!`,
    hint: "Scegli un Pokémon per vedere le sue debolezze e resistenze.",
    weak4: "4× Debole contro",
    weak2: "2× Debole contro",
    resist2: "0,5× Resiste a",
    resist4: "0,25× Resiste molto a",
    immune: "0× Immune a",
    attack: "Attacco",
    defense: "Difesa",
    statsHeading: "Stats",
    defenderWeakHeading: "Debolezze in difesa",
    teamStrengthHeading: "Punti di forza contro la mia squadra",
    matrixHeading: "Matrice dei tipi",
    legendOffense: "Verde = colpisci forte · Rosso = colpisci debolmente · Nero = nessun effetto",
    legendDefense: "Verde = la tua squadra subisce poco · Rosso = la tua squadra subisce molto · Nero = immune",
    addCard: "Aggiungi avversario",
  },
  tms: {
    heading: "Compatibilità MT",
    searchPlaceholder: "Cerca mossa …",
    noResults: "Nessuna mossa trovata",
    hint: "Scegli una mossa per vedere quali dei tuoi Pokémon possono impararla ufficialmente.",
    caveat: "Fonte: dati ufficiali del gioco (compatibilità originale), non l'assegnazione casuale della ROM.",
    notLearnable: "Questa mossa non è una MT/MN né una mossa da insegnamosse nel gioco originale.",
    empty: "Nessun Pokémon vivo catturato finora.",
    teamHeading: "Squadra",
    bankHeading: "Panchina",
    method: { tm: "MT", hm: "MN", tutor: "Tutor", level: "Liv." },
    noMoveData: "Per questo gioco non ci sono dati su MT e mosse.",
  },
  overview: {
    heading: "Panoramica",
    progress: "Progresso",
    encounterProgress: "Incontri",
    battleProgress: "Viaggio",
    badgesHeading: "Medaglie",
    badgesEmpty: "Nessuna medaglia configurata per questo gioco.",
    dashboard: "Dashboard",
    coverage: "Copertura squadra",
    offensiveGaps: "Lacune offensive",
    offensiveGapsInfo: (level: number) =>
      `Contano solo i tipi d'attacco che la squadra ha effettivamente imparato entro l'ultimo tetto di livello superato (livello ${level}).`,
    noGaps: "Nessuna lacuna: la squadra colpisce ogni tipo in modo super efficace.",
    offensiveHint: "Tipi che nessun membro colpisce in modo super efficace.",
    memorial: "Cimitero",
    memorialEmpty: "Ancora nessuna perdita. Stai attento.",
    deathPoint: "Momento della morte",
    deathPointNone: "Nessun momento",
    deathPointUnset: "Indica il momento …",
    totalDeaths: "Morti totali",
    teamBst: "BST squadra (attuale → max)",
    levelCap: "Tetto di livello (attuale → prossimo)",
    deathTallyHeading: "Responsabile delle morti",
    deathTallyUnattributed: (n: number) => `+${n} non assegnate`,
    player: "Giocatore",
    team: "Squadra",
    caught: "Catturati",
    bankBstCol: "BST panchina",
    bankBstColHint: "BST dei Pokémon catturati e vivi fuori squadra (attuale → completamente evoluto)",
    teamBstCol: "BST squadra",
    teamBstColHint: "BST dei Pokémon della squadra (attuale → completamente evoluto)",
    emptyRun: "Ancora nessun Pokémon catturato.",
    emptyRunHint: "Appena catturi qualcosa nella scheda Incontri, qui compaiono BST, copertura e statistiche.",
    bankBst: "BST in box (ora → max)",
    caughtTotal: "Catturati",
  },
  actions: {
    unknownPokemon: (id: number) => `ID Pokémon sconosciuto: ${id}`,
    unknownRoute: (id: number) => `ID percorso sconosciuto: ${id}`,
    speciesLocked: (pokemonName: string, playerLabel: string, routeName: string) =>
      `${pokemonName} è bloccato dalla Species Clause (già usato da ${playerLabel} su ${routeName}).`,
    playerNotInRun: "Questo run non ha quel giocatore.",
    encounterNotFound: (id: number) => `Incontro #${id} non trovato.`,
    soulLinkNotFound: (id: number) => `SoulLink #${id} non trovato.`,
    runNotFound: "Run non trovato: il link non è valido o il run è stato eliminato.",
    notOnPublicInstance: "Non è disponibile sul sito pubblico.",
    deadCannotEvolve: "Un Pokémon morto non può evolversi.",
    deadCannotRevert: "Non si può annullare l'evoluzione di un Pokémon morto.",
    invalidEvolutionTarget: "Obiettivo di evoluzione non valido.",
    invalidFormTarget: "Forma non valida per questo Pokémon.",
    evolutionFamilyMismatch: "Errore interno: incoerenza di famiglia in evolutions.json.",
    noPreEvolution: "Nessuna pre-evoluzione.",
    unknownLevelCap: (id: number) => `Level cap sconosciuto: ${id}`,
    presetNotFound: (id: number) => `Regolamento sconosciuto: ${id}`,
    nameRequired: "Il nome non può essere vuoto.",
    backupInvalid: "File di backup non valido o danneggiato.",
    backupEmpty: "Il backup non contiene run.",
    invalidTeamSlot: "Posto squadra non valido.",
    fusionSameEncounter: "Testa e corpo devono essere due catture diverse.",
    fusionDifferentPlayer: "Testa e corpo devono appartenere allo stesso giocatore.",
    fusionNotCaught: "Entrambe le catture devono essere vive e catturate per fondersi.",
    fusionChain: "Uno dei due fa già parte di un'altra fusione.",
    fusionNotFound: "Questa carta non è una fusione.",
    fusionDead: "Una fusione morta non può essere modificata.",
    fusionSplitDisabled:
      "In questa run la regola «le fusioni selvatiche possono essere separate» è disattivata.",
    unexpected: "Errore imprevisto - l'azione non è stata eseguita. Vedi il log del server.",
  },
};

export const translations: Record<Lang, Dictionary> = { de, en, fr, es, it };
