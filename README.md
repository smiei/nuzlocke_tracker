...
- **Type Effectiveness**: weaknesses/resistances of any Pokémon (defender's perspective) plus the full type matrix, with a client-side game picker (shared with the Pokédex tab) that switches chart, dex scope, and sprites – incl. the real Gen 1 chart.
